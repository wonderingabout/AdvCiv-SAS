	...
	CvPlot* pIgnorePlot, CvUnit* pUnit) const
{
	...
	// <!-- custom: codex change: refactor reduces redundant pathfinding and keeps bonus-first priority. -->

	std::vector<CandidatePlot> candidatePlots;

	// <!-- custom: codex change: considered early-exit bonus scan; skipped to keep pathable sorting. -->
	...
	// <!-- custom: codex change: compute food difference from yields to avoid worker/settler distortion. -->
	...
	for (WorkablePlotIter it(kCity); it.hasNext(); ++it)
	{
		...
		// <!-- custom: codex change: start iValue at 10 to avoid downstream thresholds. -->
		int iValue = 10;
		...
		if (eBonus != NO_BONUS)
		{
			...
			else
			{
				...
				if (canBuild(kPlot, eBonusSpecificBuild))
				{
					...
					// <!-- custom: codex change: boost value further for food bonuses. -->
	...
	for (size_t i = 0; i < candidatePlots.size(); ++i)
	{
		...
		if (pathFinder.generatePath(*pB))
		{
			...
			// <!-- custom: codex change: iRange=0 counts only exact-target workers (no distance limit). -->
...
void CvUnitAI::AI_workerMove(/* advc.113b: */ bool bUpdateWorkersHave)
{
	...
	// <!-- custom: codex change: reduce false worker retreats while keeping basic safety. -->
...
bool CvUnitAI::AI_nextCityToImprove(CvCity const* pCity) // advc: const param
{
    ...
    FOR_EACH_CITYAI(pLoopCity, kOwner)
    {
		...
		// <!-- custom: codex change: keep AI_getBestBuild call for stability; see crash notes. -->
...
BuildTypes CvUnitAI::AI_betterPlotBuild(CvPlot const& kPlot, BuildTypes eBuild) // advc: param was CvPlot*
{
	...
	// <!-- custom: codex change: safeguard feature-removal builds to avoid wrong improvement choice. -->
