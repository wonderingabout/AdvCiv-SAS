...
public:
	...
	// <!-- custom: pOnlyPlot/bIgnorePath let maritime Worker transport test the current SAS evaluator for an off-area BFC plot before the Worker can path there; the optional chain-step output is diagnostic provenance for the eventual Worker assignment. (GPT-5.6-Sol + ChatGPT-5.6-Sol) -->
	bool AI_bestCityBuild(CvCityAI const& kCity, CvPlot** ppBestPlot = NULL, BuildTypes* peBestBuild = NULL, CvPlot* pIgnorePlot = NULL, CvUnit* pUnit = NULL, int* piBestValue = NULL, BuildTypes* peFollowupBuild = NULL, CvPlot const* pOnlyPlot = NULL, bool bIgnorePath = false, bool* pbIrrigationChainStep = NULL) const;
	...
	// <advc.159>
	int AI_currEffectiveStr(CvPlot const* pPlot = NULL, CvUnit const* pOther = NULL, bool bCountCollateral = false, int iBaseCollateral = 0, bool bCheckCanAttack = false, int iCurrentHP = -1, bool bAssumePromotion = false) const; // advc.139
...
protected:
	...
	// K-Mod
	CvUnit* AI_findTransport(UnitAITypes eUnitAI, MovementFlags eFlags = NO_MOVEMENT_FLAGS, int iMaxPath = MAX_INT, UnitAITypes ePassengerAI = NO_UNITAI, int iMinCargo = -1, int iMinCargoSpace = -1, int iMaxCargoSpace = -1, int iMaxCargoOurUnitAI = -1);
	// K-Mod end
	void AI_animalMove();
	void AI_settleMove();
...
// advc (comment): The boolean functions below return true iff a mission was pushed
	...
	CvCity* AI_pickTargetCity(MovementFlags eFlags = NO_MOVEMENT_FLAGS, int iMaxPathTurns = MAX_INT, bool bHuntBarbs = false);
	bool AI_goToTargetCity(MovementFlags eFlags = NO_MOVEMENT_FLAGS, int iMaxPathTurns = MAX_INT, CvCity* pTargetCity = NULL);
	//bool AI_goToTargetBarbCity(int iMaxPathTurns = 10); // disabled by K-Mod. (duplicate code ftl)
	bool AI_pillageAroundCity(CvCity* pTargetCity, int iBonusValueThreshold = 0, MovementFlags eFlags = NO_MOVEMENT_FLAGS, int iMaxPathTurns = MAX_INT);
	bool AI_bombardCity();
	bool AI_cityAttack(int iRange, int iOddsThreshold, MovementFlags eFlags = NO_MOVEMENT_FLAGS, bool bFollow = false);
	bool AI_anyAttack(int iRange, int iOddsThreshold, MovementFlags eFlags = NO_MOVEMENT_FLAGS, int iMinStack = 0, bool bAllowCities = true, bool bFollow = false);
	...
	// <K-Mod>
	bool AI_defendTerritory(int iThreshold, MovementFlags eFlags, int iMaxPathTurns, bool bLocal = false);
	bool AI_stackVsStack(int iSearchRange, int iAttackThreshold, int iRiskThreshold, MovementFlags eFlags /* advc: */ = NO_MOVEMENT_FLAGS); // </K-Mod>
	bool AI_blockade();
	...
	bool AI_pillageRange(int iRange, int iBonusValueThreshold = 0, MovementFlags eFlags = NO_MOVEMENT_FLAGS);
	...
	bool AI_assaultSeaTransport(bool bAttackBarbs = false, /* K-Mod: */ bool bLocal = false, int iMaxAreaCities = MAX_INT); // advc.082
	...
	// <K-Mod>
	int AI_nukeValue(CvPlot const& kCenterPlot, int iSearchRange, CvPlot const*& pBestTarget, int iCivilianTargetWeight = 50) const; // </K-Mod>
	// <advc.121>
	int AI_connectBonusCost(CvPlot const& p, BuildTypes eBuild, int iMissingWorkersInArea) const;
	...
	bool AI_poach();
	bool AI_choke(int iRange = 1, bool bDefensive = false, MovementFlags eFlags = NO_MOVEMENT_FLAGS);
