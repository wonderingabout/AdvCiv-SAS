...
#include "BBAILog.h"
#include "SASGameRecordLog.h" // <!-- custom: Structured player/action boundaries feed factual SASGameRecord lifecycle rows. (ChatGPT-5.6-Sol) -->
...
void CvPlayer::raze(CvCity& kCity) // advc: param was CvCity*
{
	...
	// <!-- custom: A later/manual raze starts its own causal transaction, while a synchronous AI/auto raze during acquireCity deliberately joins the existing CITY_ACQUISITION transaction. (ChatGPT-5.6-Sol) -->
	SASGameRecordTransactionScope kSASCityRazeTransaction("CITY_RAZE", gGameRecordLogLevel >= 2);

	// <!-- custom: Capture rare city-raze context before destruction and finalize after disband so the same GameRecord action contains exact land/population/victory consequences. (ChatGPT-5.6-Sol) -->
	...
	bool bNoRecursion) // advc.314
{
	...
	CvGame const& kGame = GC.getGame();
	// </advc>
	// <!-- custom: Goody outcomes are rare, but their randomized realized effects can materially alter early selfplay. Gate logging-only measurements at level 2+ and preserve the entry recursion flag before AdvCiv may reuse bNoRecursion for an upgraded free-unit result. (ChatGPT-5.6-Sol) -->
		...
		for (int iPass = 0; iPass < 2; iPass++)
		{
			...
			FOR_EACH_ADJ_PLOT(*pPlot)
			{
				...
				if (iPass > 0 || SyncRandSuccess100(iProb))
				{
					...
					if(eLoopUnit != NO_UNIT)
					{
						CvUnit* pBarbarian = GET_PLAYER(BARBARIAN_PLAYER).initUnit(eLoopUnit,
								pAdj->getX(), pAdj->getY(),
	...
	for (int i = 0; i < iUnitsRequired; i++)
	{
		...
		if (pBestUnit != NULL)
		{
			...
			if (gGameRecordLogLevel >= 2)
				logSASGameRecordGreatPersonGoldenAgeConsumed(pBestUnit);
			pBestUnit->kill(true);
			//play animations
