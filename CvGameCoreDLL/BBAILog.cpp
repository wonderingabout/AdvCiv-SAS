...
#include "BBAILog.h"
#include "CvCity.h" // <!-- custom: Needed by game-summary city action/BFC rows; BBAILog.h only forward-declares CvCity. (ChatGPT-5.5) -->
#include "CvUnit.h" // <!-- custom: Needed by game-summary battle rows; BBAILog.h only forward-declares CvUnit. (ChatGPT-5.5) -->
...
static void resetSASBBAIGameSummaryState();
static void logSASBBAIGameSummaryInitialContext();
...
{
	rollSASBBAILog("new");
	if (gGameSummaryLogLevel > 0) resetSASBBAIGameSummaryState();
...
void logSASBBAINewGameStarted()
{
	...
	if (gGameSummaryLogLevel > 0) logSASBBAIGameSummaryInitialContext();
}
...
static void logSASBBAIGameSummaryTeamContacts(TeamTypes eTeam, int iGameTurn, const char* szReason)
{
	...
	logBBAI("GAME_SUMMARY_CONTACTS turn=%d reason=%s team=%d deltaValid=%d metCount=%d metCountDelta=%+d metTeams=%s",
...
static void logSASBBAIGameSummaryKnownArea(PlayerTypes ePlayer, const char* szReason)
{
	...
	logBBAI("GAME_SUMMARY_KNOWN_AREA_TYPES turn=%d reason=%s player=%d team=%d revealedTerrains=%s visibleTerrains=%s revealedFeatures=%s visibleFeatures=%s revealedBonuses=%s visibleBonuses=%s revealedImprovements=%s visibleImprovements=%s revealedRoutes=%s visibleRoutes=%s",
...
static void logSASBBAIGameSummaryStartingUnits(PlayerTypes ePlayer, const char* szReason)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	...
	for (CvUnit const* pLoopUnit = kPlayer.firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = kPlayer.nextUnit(&iLoop))
	{
		...
		logBBAI("GAME_SUMMARY_STARTING_UNIT turn=%d reason=%s player=%d unitId=%d unit=%s unitAI=%s unitCombat=%s x=%d y=%d damage=%d xp=%d level=%d movesLeft=%d plotOwner=%d plotTerrain=%s plotFeature=%s plotBonus=%s plotImprovement=%s plotRoute=%s",
...
static void logSASBBAIGameSummaryCityBFC(CvCity const& kCity, const char* szReason)
{
	CvString szTerrains;
	CvString szFeatures;
	CvString szBonuses;
...
static void logSASBBAIGameSummaryPlayerSetup(PlayerTypes ePlayer)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	CvInitCore const& kInitCore = GC.getInitCore();
	...
	logBBAI("GAME_SUMMARY_PLAYER_SETUP turn=%d player=%d team=%d alive=%d everAlive=%d human=%d slotStatus=%d playerName=%S civType=%s civName=%S civShortName=%S leaderType=%s leaderName=%S favoriteCivic=%s handicap=%s",
...
static void logSASBBAIGameSummaryAttitudeLegend()
{
	...
	logBBAI("GAME_SUMMARY_ATTITUDE_LEGEND valueFrom=AI_getAttitudeVal furious=<=%d annoyed=%d..%d cautious=%d..%d pleased=%d..%d friendly=>=%d",
			iFuriousMax, iFuriousMax + 1, iAnnoyedMax, iAnnoyedMax + 1, iPleasedMin - 1, iPleasedMin, iFriendlyMin - 1, iFriendlyMin);
}

static void logSASBBAIGameSummaryInitialContext()
{
	if (gGameSummaryLogLevel >= 2) logSASBBAIGameSummaryAttitudeLegend();
	for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		PlayerTypes eLoopPlayer = (PlayerTypes)iI;
		CvPlayer const& kLoopPlayer = GET_PLAYER(eLoopPlayer);
		if (kLoopPlayer.isEverAlive() && !kLoopPlayer.isBarbarian())
			logSASBBAIGameSummaryPlayerSetup(eLoopPlayer);
	}
	...
	for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		...
		if (!kLoopPlayer.isAlive() || kLoopPlayer.isBarbarian())
			continue;
		logSASBBAIGameSummaryKnownArea(eLoopPlayer, "setup");
		logSASBBAIGameSummaryStartingUnits(eLoopPlayer, "setup");
		int iLoop = 0;
		for (CvCity const* pLoopCity = kLoopPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kLoopPlayer.nextCity(&iLoop))
			logSASBBAIGameSummaryCityBFC(*pLoopCity, "setup");
	}
	for (int iI = 0; iI < MAX_CIV_TEAMS; iI++)
	{
		TeamTypes eLoopTeam = (TeamTypes)iI;
		if (GET_TEAM(eLoopTeam).isAlive() && !GET_TEAM(eLoopTeam).isBarbarian())
			logSASBBAIGameSummaryTeamContacts(eLoopTeam, GC.getGame().getGameTurn(), "setup");
	}
}

static void logSASBBAIGameSummaryMapBonusTotals(int iGameTurn)
{
	CvString szBonuses;
	FOR_EACH_ENUM(Bonus)
		appendSASGameSummaryTypeCount(szBonuses, getSASGameSummaryBonusType(eLoopBonus), GC.getMap().getNumBonuses(eLoopBonus));
	logBBAI("GAME_SUMMARY_MAP_BONUSES turn=%d total=%s", iGameTurn, getSASGameSummaryOrDash(szBonuses).GetCString());
}

static void logSASBBAIGameSummaryTeamProjects(TeamTypes eTeam, int iGameTurn);
...
static void logSASBBAIGameSummaryBattleBuckets(int iGameTurn)
{
	...
	for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		...
		if (g_aiSASGameSummaryBattleWins[iI] == 0 && g_aiSASGameSummaryBattleLosses[iI] == 0 && g_aiSASGameSummaryCityBattleWins[iI] == 0 && g_aiSASGameSummaryCityBattleLosses[iI] == 0)
			continue;
		logBBAI("GAME_SUMMARY_BATTLE_SUMMARY turn=%d range=%d-%d player=%d wins=%d losses=%d cityPlotWins=%d cityPlotLosses=%d",
...
static void logSASBBAIGameSummaryTeamSnapshot(TeamTypes eTeam, int iGameTurn)
{
	...
	logBBAI("GAME_SUMMARY_TEAM turn=%d team=%d members=%s alive=%d deltaValid=%d techs=%d techsDelta=%+d techEraCounts=%s land=%d landDelta=%+d landPctX100=%d landPctX100Delta=%+d pop=%d popDelta=%+d popPctX100=%d popPctX100Delta=%+d wars=%s vassals=%s master=%d",
	...
	if (gGameSummaryLogLevel >= 2) logSASBBAIGameSummaryTeamContacts(eTeam, iGameTurn, "snapshot");
	...
	FOR_EACH_ENUM(Victory)
	{
		...
		if (iLandNeed > 0 || iPopNeed > 0)
			logBBAI("GAME_SUMMARY_VICTORY_PROGRESS turn=%d team=%d victory=%s landPctX100=%d landNeed=%d popPctX100=%d popNeed=%d countdown=%d", iGameTurn, eTeam, GC.getInfo(eLoopVictory).getType(), iLandPctX100, iLandNeed, iPopPctX100, iPopNeed, kTeam.getVictoryCountdown(eLoopVictory));
	}
	if (gGameSummaryLogLevel >= 2) logSASBBAIGameSummaryTeamProjects(eTeam, iGameTurn);
}
...
static void logSASBBAIGameSummaryPolicies(PlayerTypes ePlayer, int iGameTurn)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	logBBAI("GAME_SUMMARY_POLICIES turn=%d player=%d civics=%s favoriteCivic=%s stateReligion=%s cityReligions=%s cityCorporations=%s",
...
static void logSASBBAIGameSummaryEspionage(PlayerTypes ePlayer, int iGameTurn)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	...
	logBBAI("GAME_SUMMARY_ESPIONAGE turn=%d player=%d team=%d espionageRate=%d espionagePercent=%d teamEP=%d unspentEP=%d weights=%s pointsAgainst=%s modifiers=%s",
	...
	logBBAI("GAME_SUMMARY_ESPIONAGE_DELTAS turn=%d player=%d deltaValid=%d espionageRateDelta=%+d espionagePercentDelta=%+d teamEPDelta=%+d unspentEPDelta=%+d",
...
static void logSASBBAIGameSummaryDemographics(PlayerTypes ePlayer, int iGameTurn)
{
	CvGame const& kGame = GC.getGame();
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	SASGameSummaryPlayerPrevious& kPrevious = g_akSASGameSummaryPlayerPrevious[ePlayer];
	...
	logBBAI("GAME_SUMMARY_DEMOGRAPHICS turn=%d player=%d rank=%d score=%d population=%d land=%d food=%d production=%d commerce=%d research=%d culture=%d espionage=%d goldRate=%d power=%d",
	...
	logBBAI("GAME_SUMMARY_DEMOGRAPHICS_DELTAS turn=%d player=%d deltaValid=%d scoreDelta=%+d populationDelta=%+d landDelta=%+d foodDelta=%+d productionDelta=%+d commerceDelta=%+d researchDelta=%+d cultureDelta=%+d espionageDelta=%+d goldRateDelta=%+d powerDelta=%+d",
...
static void logSASBBAIGameSummaryAttitudes(PlayerTypes ePlayer, int iGameTurn)
{
	...
	logBBAI("GAME_SUMMARY_ATTITUDES turn=%d player=%d towardValues=%s", iGameTurn, ePlayer, getSASGameSummaryOrDash(szToward).GetCString());
}

static void logSASBBAIGameSummaryDiploStatus(PlayerTypes ePlayer, int iGameTurn)
{
	CvPlayerAI const& kPlayer = GET_PLAYER(ePlayer);
	CvTeam const& kTeam = GET_TEAM(kPlayer.getTeam());
	...
	logBBAI("GAME_SUMMARY_DIPLO_STATUS turn=%d player=%d team=%d worstEnemyTeam=%d worstEnemyPlayers=%s worstEnemyOfTeams=%s atWar=%s openBorders=%s defensivePacts=%s forcePeace=%s canContact=%s canContactWilling=%s wontTalkTo=%s wontTalkFrom=%s",
...
static void logSASBBAIGameSummaryEnvironment(int iGameTurn)
{
	CvMap const& kMap = GC.getMap();
	...
	logBBAI("GAME_SUMMARY_ENVIRONMENT turn=%d globalWarmingIndex=%d globalWarmingChances=%d land=%d water=%d ownedLand=%d unownedLand=%d negativeHealthFeatures=%s features=%s",
	...
	logBBAI("GAME_SUMMARY_ENVIRONMENT_DELTAS turn=%d deltaValid=%d globalWarmingIndexDelta=%+d globalWarmingChancesDelta=%+d ownedLandDelta=%+d unownedLandDelta=%+d",
...
static void logSASBBAIGameSummaryVoteSources(int iGameTurn)
{
	CvGame const& kGame = GC.getGame();
	FOR_EACH_ENUM(VoteSource)
	{
		...
		if (pSourceCity == NULL && eReligion == NO_RELIGION && eSecretary == NO_TEAM && szVotingTeams.empty() && szVictoryVotes.empty())
			continue;
		logBBAI("GAME_SUMMARY_DIPLO_VOTE_SOURCE turn=%d source=%s secretaryTeam=%d secretaryTimer=%d voteTimer=%d religion=%s sourceOwner=%d sourceCityId=%d sourceCity=%S sourceX=%d sourceY=%d votingTeams=%s fullTeams=%s votes=%s victoryVotes=%s",
...
static void logSASBBAIGameSummaryTeamProjects(TeamTypes eTeam, int iGameTurn)
{
	...
	if (!szProjects.empty())
		logBBAI("GAME_SUMMARY_TEAM_PROJECTS turn=%d team=%d projects=%s", iGameTurn, eTeam, szProjects.GetCString());
}

static void logSASBBAIGameSummaryPlayerBonuses(PlayerTypes ePlayer, int iGameTurn, SASGameSummaryPlayerPrevious const& kPrevious)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	...
	logBBAI("GAME_SUMMARY_BONUSES turn=%d player=%d deltaValid=%d bonusTypes=%d bonusTypesDelta=%+d bonusInstances=%d bonusInstancesDelta=%+d imports=%d importsDelta=%+d exports=%d exportsDelta=%+d",
	...
	logBBAI("GAME_SUMMARY_BONUSES_AVAILABLE turn=%d player=%d available=%s", iGameTurn, ePlayer, getSASGameSummaryOrDash(szAvailable).GetCString());
	logBBAI("GAME_SUMMARY_BONUSES_TRADEABLE turn=%d player=%d tradeable=%s", iGameTurn, ePlayer, getSASGameSummaryOrDash(szTradeable).GetCString());
	logBBAI("GAME_SUMMARY_BONUSES_IMPORT_EXPORT turn=%d player=%d imported=%s exported=%s", iGameTurn, ePlayer, getSASGameSummaryOrDash(szImports).GetCString(), getSASGameSummaryOrDash(szExports).GetCString());
}
...
static void logSASBBAIGameSummaryUnitPosture(PlayerTypes ePlayer, int iGameTurn)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	TeamTypes eTeam = kPlayer.getTeam();
	SASGameSummaryPlayerPrevious& kPrevious = g_akSASGameSummaryPlayerPrevious[ePlayer];
	...
	logBBAI("GAME_SUMMARY_UNIT_POSTURE turn=%d player=%d total=%d military=%d landMilitary=%d seaMilitary=%d airMilitary=%d workers=%d settlers=%d recon=%d cityDefenders=%d fieldArmy=%d ownTerritory=%d enemyTerritory=%d neutralTerritory=%d unitsInCities=%d enemyUnitsInTerritory=%d totalXP=%d avgXpX100=%d maxXP=%d promotionReady=%d level2Plus=%d level4Plus=%d level6Plus=%d promotionInstances=%d",
	...
	logBBAI("GAME_SUMMARY_UNIT_POSTURE_DELTAS turn=%d player=%d deltaValid=%d totalDelta=%+d militaryDelta=%+d workersDelta=%+d settlersDelta=%+d fieldArmyDelta=%+d cityDefendersDelta=%+d enemyUnitsInTerritoryDelta=%+d totalXPDelta=%+d promotionReadyDelta=%+d",
	...
	logBBAI("GAME_SUMMARY_UNIT_COMPOSITION turn=%d player=%d unitAI=%s unitCombat=%s", iGameTurn, ePlayer, getSASGameSummaryOrDash(szUnitAI).GetCString(), getSASGameSummaryOrDash(szUnitCombat).GetCString());
	if (gGameSummaryLogLevel >= 3) logBBAI("GAME_SUMMARY_UNIT_PROMOTIONS turn=%d player=%d promotions=%s", iGameTurn, ePlayer, getSASGameSummaryOrDash(szPromotions).GetCString());
}

static void logSASBBAIGameSummaryWorkers(PlayerTypes ePlayer, int iGameTurn)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	TeamTypes eTeam = kPlayer.getTeam();
	SASGameSummaryPlayerPrevious& kPrevious = g_akSASGameSummaryPlayerPrevious[ePlayer];
	...
	for (CvUnit const* pLoopUnit = kPlayer.firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = kPlayer.nextUnit(&iLoop))
	{
		...
		{
			logBBAI("GAME_SUMMARY_WORKER turn=%d player=%d unitId=%d unit=%s unitAI=%s x=%d y=%d mission=%s build=%s buildTurnsLeft=%d plotOwner=%d plotTerrain=%s plotFeature=%s plotBonus=%s plotImprovement=%s plotRoute=%s guarded=%d threatened=%d",
	...
	logBBAI("GAME_SUMMARY_WORKERS turn=%d player=%d workers=%d seaWorkers=%d idle=%d building=%d buildingImprovement=%d buildingRoute=%d moving=%d waiting=%d ownTerritory=%d enemyTerritory=%d neutralTerritory=%d guarded=%d unguarded=%d threatened=%d builds=%s",
	...
	logBBAI("GAME_SUMMARY_WORKERS_DELTAS turn=%d player=%d deltaValid=%d workersDelta=%+d buildingDelta=%+d idleDelta=%+d movingDelta=%+d waitingDelta=%+d threatenedDelta=%+d",
...
static void logSASBBAIGameSummarySettlers(PlayerTypes ePlayer, int iGameTurn)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	TeamTypes eTeam = kPlayer.getTeam();
	SASGameSummaryPlayerPrevious& kPrevious = g_akSASGameSummaryPlayerPrevious[ePlayer];
	...
	for (CvUnit const* pLoopUnit = kPlayer.firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = kPlayer.nextUnit(&iLoop))
	{
		...
		if (gGameSummaryLogLevel >= 3 && pPlot != NULL)
		{
			...
			logBBAI("GAME_SUMMARY_SETTLER turn=%d player=%d unitId=%d unit=%s unitAI=%s x=%d y=%d mission=%s plotOwner=%d plotTerrain=%s plotFeature=%s plotBonus=%s plotImprovement=%s plotRoute=%s guarded=%d threatened=%d nearestCityId=%d nearestCity=%S nearestCityDistance=%d",
	...
	logBBAI("GAME_SUMMARY_SETTLERS turn=%d player=%d settlers=%d foundMission=%d moving=%d idle=%d waiting=%d ownTerritory=%d enemyTerritory=%d neutralTerritory=%d guarded=%d unguarded=%d threatened=%d",
	...
	logBBAI("GAME_SUMMARY_SETTLERS_DELTAS turn=%d player=%d deltaValid=%d settlersDelta=%+d foundMissionDelta=%+d movingDelta=%+d idleDelta=%+d waitingDelta=%+d threatenedDelta=%+d",
...
static void logSASBBAIGameSummaryCityDetail(CvCity const& kCity, int iGameTurn)
{
	...
	CvPlayer const& kOwner = GET_PLAYER(kCity.getOwner());
	logBBAI("GAME_SUMMARY_CITY turn=%d player=%d cityId=%d city=%S x=%d y=%d pop=%d foodSurplus=%d happySurplus=%d healthSurplus=%d food=%d prod=%d commerce=%d garrison=%d connectedToCapital=%d plotGroupId=%d tradeRoutes=%d domesticTradeRoutes=%d foreignTradeRoutes=%d tradeFood=%d tradeProd=%d tradeCommerce=%d productionKind=%s production=%s productionTurns=%d productionStored=%d productionNeeded=%d overflowProduction=%d featureProduction=%d specialists=%s freeSpecialists=%s gpProgress=%d gpThreshold=%d gpRate=%d gpTurnsLeft=%d gpOdds=%s",
	...
	logBBAI("GAME_SUMMARY_CITY_HAPPINESS turn=%d player=%d cityId=%d happy=%d unhappy=%d surplus=%d happySources=%s flatUnhappySources=%s angerPercentSources=%s",
	...
	logBBAI("GAME_SUMMARY_CITY_HEALTH turn=%d player=%d cityId=%d goodHealth=%d badHealth=%d surplus=%d healthySources=%s unhealthySources=%s",
	...
	if (gGameSummaryLogLevel >= 3) logBBAI("GAME_SUMMARY_CITY_TRADE_PARTNERS turn=%d player=%d cityId=%d partners=%s", iGameTurn, kCity.getOwner(), kCity.getID(), getSASGameSummaryCityTradePartners(kCity).GetCString());
}

static void logSASBBAIGameSummaryCities(PlayerTypes ePlayer, int iGameTurn)
{
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	SASGameSummaryPlayerPrevious& kPrevious = g_akSASGameSummaryPlayerPrevious[ePlayer];
	int iCities = 0;
	...
	logBBAI("GAME_SUMMARY_CITIES_DELTAS turn=%d player=%d deltaValid=%d citiesDelta=%+d connectedToCapitalDelta=%+d totalFoodSurplusDelta=%+d totalHappySurplusDelta=%+d totalHealthSurplusDelta=%+d totalFoodDelta=%+d totalProdDelta=%+d totalCommerceDelta=%+d tradeRoutesDelta=%+d tradeCommerceDelta=%+d specialistsDelta=%+d freeSpecialistsDelta=%+d garrisonDelta=%+d",
...
static void logSASBBAIGameSummaryPlayerSnapshot(PlayerTypes ePlayer, int iGameTurn)
{
	CvGame const& kGame = GC.getGame();
	CvPlayer const& kPlayer = GET_PLAYER(ePlayer);
	CvTeam const& kTeam = GET_TEAM(kPlayer.getTeam());
	...
	if (gGameSummaryLogLevel >= 2)
	{
		logSASBBAIGameSummaryPlayerBonuses(ePlayer, iGameTurn, kPrevious);
		logSASBBAIGameSummaryPolicies(ePlayer, iGameTurn);
		logSASBBAIGameSummaryEspionage(ePlayer, iGameTurn);
		logSASBBAIGameSummaryDemographics(ePlayer, iGameTurn);
		logSASBBAIGameSummaryAttitudes(ePlayer, iGameTurn);
		logSASBBAIGameSummaryDiploStatus(ePlayer, iGameTurn);
		logSASBBAIGameSummaryUnitPosture(ePlayer, iGameTurn);
		logSASBBAIGameSummaryWorkers(ePlayer, iGameTurn);
		logSASBBAIGameSummarySettlers(ePlayer, iGameTurn);
		logSASBBAIGameSummaryCities(ePlayer, iGameTurn);
	}
	kPrevious.bValid = true;
...
{
	CvGame const& kGame = GC.getGame();
	logBBAI("GAME_SUMMARY_TURN_BEGIN turn=%d elapsed=%d year=%d playersAlive=%d teamsAlive=%d totalCities=%d totalPopulation=%d",
	...
	if (gGameSummaryLogLevel >= 2)
	{
		logSASBBAIGameSummaryMapBonusTotals(iGameTurn);
		logSASBBAIGameSummaryEnvironment(iGameTurn);
		logSASBBAIGameSummaryVoteSources(iGameTurn);
	}
	for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		PlayerTypes eLoopPlayer = (PlayerTypes)iI;
		if (GET_PLAYER(eLoopPlayer).isAlive() && !GET_PLAYER(eLoopPlayer).isBarbarian())
			logSASBBAIGameSummaryPlayerSnapshot(eLoopPlayer, iGameTurn);
	}
	for (int iI = 0; iI < MAX_CIV_TEAMS; iI++)
	{
		TeamTypes eLoopTeam = (TeamTypes)iI;
		if (GET_TEAM(eLoopTeam).isAlive() && !GET_TEAM(eLoopTeam).isBarbarian())
			logSASBBAIGameSummaryTeamSnapshot(eLoopTeam, iGameTurn);
	}
	if (gGameSummaryLogLevel >= 2) logSASBBAIGameSummaryBattleBuckets(iGameTurn);
	logBBAI("GAME_SUMMARY_TURN_END turn=%d", iGameTurn);
}
...
{
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=TECH_ACQUIRED player=%d team=%d tech=%s", GC.getGame().getGameTurn(), ePlayer, eTeam, getSASGameSummaryTechType(eType));
}
...
void logSASBBAIGameSummaryCityBuilt(CvCity const* pCity)
{
	if (pCity == NULL)
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=CITY_BUILT player=%d cityId=%d city=%S x=%d y=%d pop=%d",
			GC.getGame().getGameTurn(), pCity->getOwner(), pCity->getID(), pCity->getName().GetCString(), pCity->getX(), pCity->getY(), pCity->getPopulation());
	if (gGameSummaryLogLevel >= 2) logSASBBAIGameSummaryCityBFC(*pCity, "built");
}
...
void logSASBBAIGameSummaryCityRazed(CvCity const* pCity, PlayerTypes ePlayer)
{
	if (pCity == NULL)
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=CITY_RAZED player=%d oldOwner=%d cityId=%d city=%S x=%d y=%d pop=%d",
...
void logSASBBAIGameSummaryCityAcquired(PlayerTypes eOldOwner, PlayerTypes eNewOwner, CvCity const* pCity, bool bConquest, bool bTrade)
{
	if (pCity == NULL)
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=CITY_ACQUIRED oldOwner=%d newOwner=%d cityId=%d city=%S x=%d y=%d pop=%d conquest=%d trade=%d",
	...
	if (gGameSummaryLogLevel >= 2) logSASBBAIGameSummaryCityBFC(*pCity, "acquired");
}

void logSASBBAIGameSummaryChangeWar(bool bWar, TeamTypes eTeam, TeamTypes eOtherTeam)
{
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=%s teamA=%d teamB=%d", GC.getGame().getGameTurn(), bWar ? "WAR_STARTED" : "WAR_ENDED", eTeam, eOtherTeam);
}
...
void logSASBBAIGameSummaryTeamMet(TeamTypes eTeam, TeamTypes eOtherTeam, bool bNewDiplo, int iX1, int iY1, int iX2, int iY2, CvPlot const* pTeamContactPlot, CvPlot const* pOtherContactPlot)
{
	...
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=TEAM_MET team=%d otherTeam=%d bNewDiplo=%d teamMembers=%s otherMembers=%s meetDataPlot1=%d,%d meetDataPlot1Valid=%d meetDataPlot2=%d,%d meetDataPlot2Valid=%d teamContactPlot=%d,%d otherTeamContactPlot=%d,%d",
...
{
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=GOLD_TRADE from=%d to=%d amount=%d", GC.getGame().getGameTurn(), eFromPlayer, eToPlayer, iAmount);
}

void logSASBBAIGameSummaryReligionFounded(ReligionTypes eReligion, PlayerTypes ePlayer)
{
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=RELIGION_FOUNDED player=%d religion=%s", GC.getGame().getGameTurn(), ePlayer, getSASGameSummaryReligionType(eReligion));
}

void logSASBBAIGameSummaryCorporationFounded(CorporationTypes eCorporation, PlayerTypes ePlayer)
{
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=CORPORATION_FOUNDED player=%d corporation=%s", GC.getGame().getGameTurn(), ePlayer, getSASGameSummaryCorporationType(eCorporation));
}
...
void logSASBBAIGameSummaryBuildingBuilt(CvCity const* pCity, BuildingTypes eBuilding)
{
	if (pCity == NULL || eBuilding == NO_BUILDING || !GC.getInfo(eBuilding).isLimited())
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=WONDER_BUILT player=%d cityId=%d city=%S building=%s", GC.getGame().getGameTurn(), pCity->getOwner(), pCity->getID(), pCity->getName().GetCString(), getSASGameSummaryBuildingType(eBuilding));
}
...
void logSASBBAIGameSummaryProjectBuilt(CvCity const* pCity, ProjectTypes eProject)
{
	if (pCity == NULL)
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=PROJECT_BUILT player=%d cityId=%d city=%S project=%s", GC.getGame().getGameTurn(), pCity->getOwner(), pCity->getID(), pCity->getName().GetCString(), getSASGameSummaryProjectType(eProject));
}

void logSASBBAIGameSummaryVassalState(TeamTypes eMaster, TeamTypes eVassal, bool bVassal)
{
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=%s master=%d vassal=%d", GC.getGame().getGameTurn(), bVassal ? "VASSALAGE_STARTED" : "VASSALAGE_ENDED", eMaster, eVassal);
}

void logSASBBAIGameSummaryVictory(TeamTypes eWinner, VictoryTypes eVictory)
{
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=VICTORY team=%d victory=%s", GC.getGame().getGameTurn(), eWinner, eVictory == NO_VICTORY ? "-" : GC.getInfo(eVictory).getType());
}

void logSASBBAIGameSummaryGreatPersonBorn(CvUnit const* pUnit, PlayerTypes ePlayer, CvCity const* pCity)
{
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=GREAT_PERSON_BORN player=%d cityId=%d city=%S unit=%s combatXP=%d greatPeopleCreated=%d greatGeneralsCreated=%d greatGeneralThreshold=%d",
...
void logSASBBAIGameSummaryGreatPersonJoined(CvUnit const* pUnit, CvCity const* pCity, SpecialistTypes eSpecialist)
{
	if (pUnit == NULL || pCity == NULL)
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=GREAT_PERSON_JOINED_CITY player=%d unitId=%d unit=%s cityId=%d city=%S specialist=%s freeSpecialists=%d",
...
void logSASBBAIGameSummaryGreatGeneralAttached(CvUnit const* pGreatGeneral, CvUnit const* pTargetUnit, PromotionTypes ePromotion)
{
	if (pGreatGeneral == NULL || pTargetUnit == NULL)
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=GREAT_GENERAL_ATTACHED player=%d generalUnitId=%d generalUnit=%s targetUnitId=%d targetUnit=%s targetUnitAI=%s x=%d y=%d promotion=%s targetXP=%d targetLevel=%d",
...
void logSASBBAIGameSummaryUnitScrapped(CvUnit const* pUnit)
{
	if (pUnit == NULL)
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=UNIT_SCRAPPED player=%d unitId=%d unit=%s unitAI=%s x=%d y=%d damage=%d xp=%d level=%d age=%d cargo=%d cargoSpace=%d",
...
void logSASBBAIGameSummaryUnitUpgraded(CvUnit const* pOldUnit, CvUnit const* pNewUnit, int iCost)
{
	if (pOldUnit == NULL || pNewUnit == NULL)
		return;
	logBBAI("GAME_SUMMARY_ACTION turn=%d type=UNIT_UPGRADED player=%d oldUnitId=%d newUnitId=%d fromUnit=%s toUnit=%s unitAI=%s x=%d y=%d cost=%d oldXP=%d newXP=%d oldLevel=%d newLevel=%d",
...
void logSASBBAIGameSummaryCombatResult(CvUnit const* pWinner, CvUnit const* pLoser)
{
	...
	{
		logBBAI("GAME_SUMMARY_ACTION turn=%d type=GREAT_GENERAL_UNIT_DIED player=%d unitId=%d unit=%s attachedGreatGeneral=%s winnerPlayer=%d winnerUnitId=%d winnerUnit=%s x=%d y=%d",
	...
	if (gGameSummaryLogLevel >= 3)
	{
		logBBAI("GAME_SUMMARY_BATTLE turn=%d winner=%d loser=%d winnerUnit=%s loserUnit=%s x=%d y=%d cityPlot=%d winnerBaseStr=%d loserBaseStr=%d winnerDamage=%d loserDamage=%d winnerLeaderUnit=%s loserLeaderUnit=%s",
