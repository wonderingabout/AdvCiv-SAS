...
- [`historical_source_archaeology.txt`](/_1_AdvCiv-SAS/Docs/Source_Analysis/historical_source_archaeology.txt) records the completed commit-oriented lineage sweep. For a new historical question, search `LLM_Helpers/context/commit_diffs/` through `INDEX.txt` or `PATH_HISTORY_INDEX.txt`, then inspect the matching diff and Git-log evidence.
...
- Within AdvCiv-SAS's mod repo, generate/use `LLM_Helpers/context/commit_diffs/` for exact inherited ancestry and [`LLM_Helpers/context/mapscript_refs/`](/LLM_Helpers/context/mapscript_refs/) for imported map-script originals.
- Shared development context lives under [`LLM_Helpers/context/`](/LLM_Helpers/context/). Normal full-history light-source ZIP generation includes freshly generated commit diffs at that same canonical path and refreshes the Git-ignored local generated copy after the ZIP succeeds; use `python LLM_Helpers/refresh_commit_diffs.py` to refresh it independently. Both context corpora are omitted from generated historical patches to prevent recursion/import noise.
