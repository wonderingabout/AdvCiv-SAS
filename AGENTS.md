...
- [`historical_source_archaeology.txt`](/_1_AdvCiv-SAS/Docs/Source_Analysis/historical_source_archaeology.txt) records the completed commit-oriented lineage sweep. For a new historical question, search [`LLM_Helpers/commit_diffs/`](/LLM_Helpers/commit_diffs/) through `INDEX.txt` or `PATH_HISTORY_INDEX.txt`, then inspect the matching diff and Git-log evidence.
...
- Within AdvCiv-SAS's mod repo, use `commit_diffs` for exact inherited ancestry and [`LLM_Helpers/map_refs/`](/LLM_Helpers/map_refs/) for imported map-script originals.
- The light-source ZIP creates its own fresh `_SNAPSHOT_CONTEXT/commit_diffs/` and includes current source-analysis records and map references for external LLM handoffs, while omitting their own revisions from generated historical patches. The tracked history mirror can lag HEAD until `python LLM_Helpers/sync_tracked_commit_diffs.py` is run and is excluded from its own historical patches to prevent recursion.
