...
Historical code archaeology: for inherited regressions, suspicious behavior changes, or questions about when code changed, use the greppable tracked history mirror in [`LLM_Helpers/commit_diffs/`](/LLM_Helpers/commit_diffs/) before guessing. Start with `INDEX.txt` or `PATH_HISTORY_INDEX.txt`, then inspect the matching commit diff and the tracked anonymized Git logs for full commit messages.

Use the source reference that matches the question. Current AdvCiv-SAS source and Git are authoritative for the mod. The locally installed latest base AdvCiv source (currently 1.14) at "C:\Program Files (x86)\Steam\steamapps\common\Sid Meier's Civilization IV Beyond the Sword\Beyond the Sword\Mods\AdvCiv" is the direct upstream reference for current base-AdvCiv behavior and should also be supplied to external LLMs when that comparison matters. Use `LLM_Helpers/commit_diffs` to trace AdvCiv/K-Mod/AdvCiv-SAS ancestry and determine when or why code changed.

For comparisons farther back against the vanilla Civ4/BtS patch lineage, a local checkout of the [Civ4CE repository](https://github.com/f1rpo/Civ4CE) is available at "C:\Program Files (x86)\Steam\steamapps\common\Sid Meier's Civilization IV Beyond the Sword\Beyond the Sword\Mods\Civ4CE-main"; the BtS SDK source is under `BtS/CvGameCoreDLL`. Its readme describes a chronological reconstruction from installer archives. This downloaded `Civ4CE-main` checkout has no `.git` metadata, so its files are useful supporting tip-state comparisons but not local commit-history evidence. It was identified after the completed archaeology sweeps and did not certify their existing findings; it supplements rather than replaces current base AdvCiv and the tracked lineage.

For imported/third-party map scripts, also compare the greppable downloaded originals in [`LLM_Helpers/map_refs/`](/LLM_Helpers/map_refs/); their README records package hashes, portable directory-name mappings and selection/provenance limits. Current source/Git remains authoritative and the tracked mirror may lag HEAD until `python LLM_Helpers/sync_tracked_commit_diffs.py` is run.

The light-source ZIP independently generates a fresh `_SNAPSHOT_CONTEXT/commit_diffs/` copy and includes the map-script reference corpus. The tracked mirror is intentionally excluded from its own historical patches so archive-refresh commits cannot recursively archive history-of-history. For the ongoing systematic inherited-bug sweep, also update [`advciv_kmod_archaeology_progress.txt`](/_1_AdvCiv-SAS/Docs/Modding_Ressources/advciv_kmod_archaeology_progress.txt).
...
- Before confirming an apparent enum/type mismatch through an accessor macro or helper, trace the active definition and overloads in the caller's include context. In AI DLL source, for example, `GET_TEAM(PlayerTypes)` is deliberately valid: `CoreAI::getTeam(PlayerTypes)` resolves through `TEAMID(ePlayer)`. KI#311 records the rejected false positive and its `TEAMREF(PlayerTypes)` -> overloaded `GET_TEAM` lineage.
