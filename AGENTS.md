...
- [`historical_source_archaeology.txt`](/_1_AdvCiv-SAS/Docs/Source_Analysis/historical_source_archaeology.txt) records the completed commit-oriented lineage sweep. For a new historical question, search [`LLM_Helpers/context/commit_diffs/`](/LLM_Helpers/context/commit_diffs/) through `INDEX.txt` or `PATH_HISTORY_INDEX.txt`, then inspect the matching diff and Git-log evidence.
- The [C++ File Audit Album](/_1_AdvCiv-SAS/Docs/Source_Analysis/cpp_file_audit_album.txt) records the ongoing current-tree sweep, including clean files, false positives, stable `F###` findings and its continuation cursor. Reconcile provisional KI numbers against current Known Issues before promotion. Future Python, XML or other file-oriented analyses belong beside it.
- Current AdvCiv-SAS source and Git are authoritative. Local comparison trees share the parent "C:\Program Files (x86)\Steam\steamapps\common\Sid Meier's Civilization IV Beyond the Sword\Beyond the Sword\Mods": use `AdvCiv` for AdvCiv 1.14, `K-Mod` for the downloaded K-Mod 1.46 master snapshot, and `Civ4CE-main/BtS/CvGameCoreDLL` for supporting vanilla/BtS tip-state comparison.
- Within AdvCiv-SAS's mod repo, use [`LLM_Helpers/context/commit_diffs/`](/LLM_Helpers/context/commit_diffs/) for exact inherited ancestry and [`LLM_Helpers/context/mapscript_refs/`](/LLM_Helpers/context/mapscript_refs/) for imported map-script originals.
- Shared development context lives under [`LLM_Helpers/context/`](/LLM_Helpers/context/). Normal full-history light-source ZIP generation includes freshly generated commit diffs at that same canonical path and refreshes the local tracked copy after the ZIP succeeds; use `python LLM_Helpers/refresh_commit_diffs.py` to refresh it independently. Both context corpora are omitted from generated historical patches to prevent recursion/import noise.
...
- "C:\Users\PC\Documents\My Games\beyond the sword\Logs\SASGameRecord.log" or timestamped (e.g., `SASGameRecord_20260710T064839Z_load1.log`) (after enabling the AdvCiv-SAS game-record XML defines). This is separate from BBAI logging: use it for compact autoplay / AI-benchmark context such as initial map/landmass geography and bonus/yield context, economy, expansion, city and battle history, worked plots, unit composition, city-defense/bombard history, tactical air use, and high-level game state; use BBAI for detailed AI-decision diagnostics. It can also be useful alongside BBAI logs to give an LLM the broader "what happened" context before drilling into decision traces. It is an all-player diagnostic record and can contain spoilers; do not treat it as a spoiler-free player-advice log.
- A full raw example is kept at [_1_AdvCiv-SAS/SASGameRecord_log/SASGameRecord_example.log](/_1_AdvCiv-SAS/SASGameRecord_log/SASGameRecord_example.log) for development/LLM review.
...
- Screenshots can help development/review by showing what the mod UI/gameplay screens and rendered `SASGameRecord` map text actually look like, not only source/docs. LLMs may parse the raw text-map characters without reconstructing the visual/geographical layout as easily, so `SASGameRecord_map_text` screenshots are useful review context too. Codex and similar local agentic tools can read these screenshot folders directly on the user's computer; external/ZIP-only LLMs such as ChatGPT depend on what the compact light source ZIP includes, so [make_light_source_zip.py](/LLM_Helpers/make_light_source_zip.py) also includes selected useful screenshot folders (advisors, main menu, `SASGameRecord_map_text` screenshots, Sevopedia, common UI).
...
- Avoid unclear wording where one word can mean several Civ4 things. For example, "citizen" can mean a population member assigned to a city plot or a `SPECIALIST_CITIZEN`; "worked" can mean a plot assigned by the city or a tile improvement or build or route or such made by a `UNITCLASS_WORKER` or `UNITCLASS_WORKBOAT` or such unit; "event" can mean a BBAI/SASGameRecord log row/action or an XML `EventInfo`/`EventTrigger`; "reserve" can mean remaining defenders or `UNITAI_RESERVE`; "warlord" can mean Great General attached-to-unit use or `HANDICAP_WARLORD`. Use the exact meaning instead, e.g. "remaining healthy defenders after giving one to the Settler"; additionally when writing user facing doc like in the `README_Main_Changes_Guide.md`, this helps make it clearer too.
...
- When a code comment documents a Known Issues fix or prepared fix, include the exact greppable reference `See KI#number.` (for example, `See KI#234.`). Treat it as a closing note: in a multiline comment, place it on the final line immediately before the model credit.
...
- AdvCiv-SAS supports only saves created by the current mod version. When the save schema changes, read and write only the current format; do not add `uiFlag` compatibility branches, migrations, fallback defaults, legacy-field handling, or tests for older saves. Save/reload within the same current version must still work.
- Current-version-only support does not mean leaving serialized format identifiers stale. When a persistent subsystem already stores its own format/version marker (e.g. `UWAICache::City::iSaveVersion`), bump that marker when its current record layout changes and update the unconditional current-format write/read order together. This truthfully identifies the new layout; it is distinct from adding `uiFlag` or version-conditional compatibility paths for older saves, which remain unwanted.
...
- For technical C++ under active crash/PDB investigation, preserve source-line structure when it helps WinDbg isolate the failing access. Function signatures and flat logging argument lists remain good collapse candidates, but complicated conditions or expressions with several independently crashable calls may use readable BtS-style multiline layout; do not combine distinct risky accesses onto one debugger source line solely for grep or compactness.
...
- Note: sometimes our files have a mix of CRLF/LF in them causing next commit to have a massive git diff (e.g., 240 insertions(+), 240 deletions(-) when there is in fact no change) when VS Code or maybe the LLM tries to next modify it, and this is a problem because it pollutes diff and makes it hard to read. But it is not a major concern worth getting distracted over; it seems the game runs fine either way. As for the cause, this is likely caused by codex's apply patching tool (git bash shows `warning: in the working copy of 'AGENTS.md', LF will be replaced by CRLF the next time Git touches it` after codex's one line modification) (likely because Civ4 was mostly written for Windows/CRLF, and Codex patch tools may favour Linux LF-style patching causing mixed lines (files usually have almost entirely CRLF, with some minor LF lines matching the files we changed)).
- Note (appendix): As for the cure, the GitHub workflow now checks this with [`build/line_endings.py`](/.github/workflows/README.md#buildline_endingspy) (e.g., `PrivateMaps/Pangaea.py: has mixed line endings: CRLF=614, LF=21, lone_CR=0`), and local reviewed cleanup can use [`fix_line_endings.py`](/LLM_Helpers/fix_line_endings.py), which preserves each file's dominant line-ending style by default. On your end, if you notice warnings such as `LF will be replaced by CRLF`, use commands like `git diff --ignore-space-at-eol` and focus on your task first rather than getting distracted or going down a rabbit hole needlessly. If needed, you can use the abovementioned resources to diagnose and fix them (because this is always nice and cleaner, but not high-priority; better make code work first, and at worst we'll just amend the commit and fix the test later).
- Before confirming an apparent enum/type mismatch through an accessor macro or helper, trace the active definition and overloads in the caller's include context. `GET_TEAM(PlayerTypes)` is deliberately valid: `CoreAI::getTeam(PlayerTypes)` and the parallel non-AI `CvGamePlay::getTeam(PlayerTypes)` resolve through `TEAMID(ePlayer)`. KI#311 and KI#387 record the respective rejected false positives and the `TEAMREF(PlayerTypes)` -> overloaded `GET_TEAM` lineage.
...
- Treat Ruff/lint output as hints only; Civ4 runs Python 2.4 and engine imports can look unused to modern linters. For actual parser compatibility, GitHub Actions now runs [`python24-compile.yml`](/.github/workflows/python24-compile.yml), which uses the Docker image from [`wonderingabout/python-2.4-docker`](https://github.com/wonderingabout/python-2.4-docker) to compile-check `Assets/Python` and `PrivateMaps` with real CPython 2.4 syntax/bytecode rules.
...
- For very messy legacy files where automated scripts produce risky or hard-to-review diffs, ask the user to send the file to ChatGPT for a direct manual repair pass. This can be better than spending agent/Codex tokens on fragile automation, especially for indentation archaeology or old map scripts. Example: the `BTG_Cross.py` and `BTG_Grid.py` map scripts were easier to repair manually as full files after broad indentation scripts produced misleading results.
...
- Prefer strict XML lookup helpers from `SASUtils` over raw lookups for static/literal tags: use `getInfoTypeOrFail("TAG")` instead of raw `gc.getInfoTypeForString("TAG")`, and use `findInfoTypeNumOrFail(gc.getXInfo, gc.getNumXInfos, "TAG")` instead of raw `CvUtil.findInfoTypeNum(...)`. This keeps missing or renamed XML tags loud instead of silently returning `-1`; it helped catch incorrect direction-string usage in Planet Generator map (e.g., `getInfoTypeOrFail("DIRECTION_NORTH")` instead of `DirectionTypes.DIRECTION_NORTH*`) and incorrect plot/terrain-style lookups (e.g., `getInfoTypeOrFail("PLOT_PEAK")` instead of `PlotTypes.PLOT_PEAK` in Peirce map). Dynamic/guarded legacy lookups can stay raw only when intentionally allowed by [`.github/workflows/build/raw_getinfotype.py`](/.github/workflows/README.md#buildraw_getinfotypepy).
...
- For map scripts, keep reusable helper modules outside `PrivateMaps` (e.g., in `Assets/Python/SAS_WorldSizeUtils.py`) when they define or might define names that Civ4 treats as map-script callbacks (`getGridSize`, `getNumPlotsPercent`, `getDescription`, etc.). The old playable `PrivateMaps/SAS_WorldSizes.py` helper leaked its compact `getGridSize` through `from SAS_WorldSizes import *`, making RandomScriptMap Huge generate empirically too small (`60 x 44`). Fix: moved helpers to `Assets/Python/SAS_WorldSizeUtils.py`; result: RandomScriptMap Huge returned to `120 x 84`. The playable sandbox wrapper is now `PrivateMaps/SAS_Simple_Flat_Grass.py`.
...
- Keep magic numbers/constants in [SASMagicNumbers.py](/Assets/Python/SASMagicNumbers.py) when they benefit from a named home, especially when they are imported across advisors, Sevopedia, and utilities. This module should stay import-free: no `CvPythonExtensions`, no `gc`, no screen helpers, and no `SASUtils`, so it remains a safe leaf module and does not create circular-dependency or load-order risk. Prefix exported names with `SAS_MAGIC_` to reduce accidental global-name collisions in files that use from-module import-star. Useful examples: `SAS_MAGIC_PEDIA_PYTHON_BUILD = 6798` is shared by Sevopedia and Tech Chooser instead of being defined twice; `SAS_MAGIC_PEDIA_PYTHON_LEADER_ATTITUDE = 6805` was originally local to Sevopedia Leader but also checked by Sevopedia Main, so centralizing avoids widget-id drift/collision.
...
- C++ signatures: prefer safe one-line function declarations/definitions for easier grep, diffs, and LLM-agent review, especially for overloaded functions like `void addMessage` where the whole signature is the useful search key. Example: a declaration like `void CvDeal::endTrade(TradeData trade, PlayerTypes eFromPlayer, PlayerTypes eToPlayer, bool bTeam, /* advc.036: */ bool bUpdateAttitude, PlayerTypes eCancelPlayer) // advc.130p` is easier to find and reason about than the old three-line equivalent. See also [collapse_cpp_signatures.py](/LLM_Helpers/README.md#collapse_cpp_signaturespy) if needed. Note: Since the C++ codebase is large, apply this as we go if deemed relevant rather than trying to fix them all right away.
