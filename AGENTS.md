...
- Current AdvCiv-SAS source and Git are authoritative. Compare current upstream behavior with local AdvCiv 1.14 at "C:\Program Files (x86)\Steam\steamapps\common\Sid Meier's Civilization IV Beyond the Sword\Beyond the Sword\Mods\AdvCiv"; use `commit_diffs` for ancestry, local `Civ4CE-main/BtS/CvGameCoreDLL` for supporting vanilla/BtS tip-state comparison, and [`LLM_Helpers/map_refs/`](/LLM_Helpers/map_refs/) for imported map-script originals.
