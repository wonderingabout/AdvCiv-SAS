...
def normalize_to_100(value, min_val, max_val, invert, attr_name=""):
	...
	- Normalizes adjusted_min / adjusted_max.
	...
	if min_val < 0:
		adjusted_min = value + abs(min_val)
		adjusted_max = max_val + abs(min_val)
	elif min_val > 0:
		adjusted_min = value - min_val
	...
	# --- Normalize ---
	if adjusted_min < 0:
		raise ValueError("Adjusted value of %s is negative before normalization: adjusted_min=%d, adjusted_max=%d, min_val=%d, max_val=%d" % (attr_name, adjusted_min, adjusted_max, min_val, max_val))

	norm = float(adjusted_min) / float(adjusted_max)

	final_score = int(round(norm * 100))
	if (final_score < 0) or (final_score > 100):
		raise ValueError("Norm of %s out of range (0-100) during normalization: final_score=%.3f, adjusted_min=%d, adjusted_max=%d, min_val=%d, max_val=%d" % (attr_name, final_score, adjusted_min, adjusted_max, min_val, max_val))
