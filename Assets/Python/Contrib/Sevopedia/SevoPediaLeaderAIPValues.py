...
def dump_leader_cache_to_pythondbg(leaders_info_cached, excluded_leader_types, is_emoji_enabled, is_raw_xml_names):
	...
	for iLeader in xrange(gc.getNumLeaderHeadInfos()):
		leader_index_type_pairs.append((iLeader, gc.getLeaderHeadInfo(iLeader).getType()))
	leader_index_type_line = format_aip_leader_index_type_pairs(leader_index_type_pairs)
	source_header_lines = (
		"# Generated by: manual in-game PythonDbg.log cache dump",
	)
	leader_count_line = "#   - Number of leaders (gc.getNumLeaderHeadInfos): %d" % gc.getNumLeaderHeadInfos()
	for line in build_aip_predump_module_lines(leaders_info_cached, leader_count_line, excluded_leader_types, leader_index_type_line, is_emoji_enabled, is_raw_xml_names, source_header_lines, False):
		emit(line)
