...
def getDescriptionTitle():
	return "A very popular BTS map which has the shape of a ring, where each player gets its own circled amount of land is linked to the other players via an external band of land"
...
def getDescriptionThird():#Option : (at the bottom)"
	return "Default the neutral territory to Islands so that boats can cross and map is more active, also has a default option to freeze those tiles"	
	
def getDescriptionScenario():#Scenario : (at the bottom)"
	return "Traditional map for 3v3 Industrial Start, also some Renaissance. Also very good map for Ironman. Don't play 3 players on this, you can play 4 and 2v2 but it will be made into a diamond shape"	
...
def getCustomMapOptionDescAt(argsList):
	...
	selection_names = {
		...
		9:	{
			0: "No",
			1: "Yes - Balanced for Lead and Nickel, Normal for others"	
			},	
		10:	{
			0: "Normal Forest",
			1: "75% Forest - 25% Palm",
...
def normalizeAddExtras():
	...
	if (CyMap().getCustomMapOption(5) == 1 or CyMap().getCustomMapOption(5) == 2):
	...
	for i in range(0,gc.getMAX_CIV_PLAYERS()):
		...
		if gc.getPlayer(i).isEverAlive():#2.22 for Spectator
	...
	if isBTPon:
		...
		if (CyMap().getCustomMapOption(9) == 1):#2.35
			listToBalance = ["BONUS_LEAD","BONUS_NICKEL"]
...
class RingTerrainGenerator(CvMapGeneratorUtil.TerrainGenerator):
	...
	#2.15
	'''def __init__(self, iDesertPercent=10, iPlainsPercent=30,
	             iIcePercent=50, iTundraPercent=35,
	             fracXExp=-1, fracYExp=-1, grain_amount=5):'''#that's legacy Green Ring
	...
	def generateTerrainAtPlot(self,iX,iY):
		...
		if ((iX >= self.cenWestX and iX <= self.cenEastX) and (iY >= self.cenSouthY and iY <= self.cenNorthY)):
			...
			elif (CyMap().getCustomMapOption(8) == 2 and isBTPon):#2.15
				if val >= self.iIceBottom:
			...
			elif (CyMap().getCustomMapOption(8) == 2 and not isBTPon):#2.15
				if val >= self.iIceBottom:
...
class RingFeatureGenerator(CvMapGeneratorUtil.FeatureGenerator):
	...
	def addIceAtPlot(self, pPlot, iX, iY):
		if (CyMap().getCustomMapOption(8) == 1):
			if not pPlot.isWater() and ((iX >= self.cenWestX and iX <= self.cenEastX) and (iY >= self.cenSouthY and iY <= self.cenNorthY)):
