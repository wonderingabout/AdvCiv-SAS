#
#	FILE:	 LD_Mirror_Inland_Sea3a.py
#	AUTHOR:  Leszek Deska ldeska@wp.pl, based on Inland_Sea and C4F_Inland_Sea 
...
# HISTORY: v.1 - initial release
#          v.2 - starting points in 25% of map width (instead of 40% in v.1)
#          v.3 - corrected to make sure that all important resources will exist
#                and will be located in reasonable distance from starting point 
#-----------------------------------------------------------------------------
...
balancer.resourcesToBalance = ('BONUS_MARBLE', 'BONUS_STONE', 'BONUS_SPICE', 'BONUS_SUGAR', 'BONUS_GOLD', 'BONUS_GEMS', 'BONUS_SILVER', 'BONUS_IVORY', 'BONUS WINE', 'BONUS_INCENSE', 'BONUS_FUR', 'BONUS_ALUMINUM', 'BONUS_COPPER', 'BONUS_HORSE', 'BONUS_IRON', 'BONUS_OIL', 'BONUS_URANIUM', 'BONUS_CORN', 'BONUS_PIG', 'BONUS_COW', 'BONUS_RICE', 'BONUS_BANANA')
stratresourcesToBalance = ('BONUS_OIL', 'BONUS_URANIUM', 'BONUS_ALUMINUM', 'BONUS_IVORY', 'BONUS_MARBLE', 'BONUS_STONE')
...
def beforeGeneration():
	...
	configs = [0, 1, 6, 4, 3, 1, 2, 2, 4, 2, 2, 2, 1, 2, 1, 2, 1, 2, 1]
...
def generateTerrainTypes():
	...
	for i in range(len(terrainTypes)):
		if (terrainTypes[i] == terraingen.terrainIce) or (terrainTypes[i] == terraingen.terrainTundra) or (terrainTypes[i] == terraingen.terrainPlains): 
...
def normalizeAddExtras():
	...
	# V2 by Axius: Give a land oil to each player
		...
		if type_string in resourcesMustHave:
			...
			if not have_it:
				...
				if not bInfo.isTerrain(p.getTerrainType()):
					tryTerrain = gc.getInfoTypeForString("TERRAIN_GRASS")
					if not bInfo.isTerrain(tryTerrain):
						tryTerrain = gc.getInfoTypeForString("TERRAIN_PLAINS")
						if not bInfo.isTerrain(tryTerrain):
							tryTerrain = gc.getInfoTypeForString("TERRAIN_DESERT")
...
def addBonusType(argsList):
	...
	if (CyMap().getCustomMapOption(1) == 1):
		...
		random.seed(gc.getGame().getMapRand().get(30000, "Shuffle Plots and Bonuses - INLAND_SEA PYTHON"))

		for i in range(0,gc.getMAX_CIV_PLAYERS()):
			if gc.getPlayer(i).isAlive():
				normalizeAddExtraExtras(i)
