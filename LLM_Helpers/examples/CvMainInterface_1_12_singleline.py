...
class CvMainInterface:
	...
	def setDefaultHelpTextArea(self, bMinMargin = False):
		...
		# (as well as fWidth), let the DLL handle the matter - which it will do by hacking the EXE. And this hack needs to happen before creating the help text area.
	...
	def stackBarDefaultTextOffset(self):
		# Was 3 or 4 in BtS. Not sure whether the text is intended to be aligned at the bottom or center - the height of the game font is almost the same as the height of the bars. Let's assume that we want to center the text.
		# I would've thought that making the offset a fraction of the bar height would accomplish that, but, somehow, it's not enough.
...
# I know that this is redundant, but CyInterface().getPlotListOffset() (and probably the column one too)
# uses this function
...
# BUG - Raw Yields - end
		...
		gSetRect("LowerLeftCornerPanel", "Top", RectLayout.LEFT, RectLayout.BOTTOM, HLEN(304), gRect("MiniMapPanel").height() + 17)
		...
		gSetRect("LowerRightCorner", "LowerRightCornerPanel", 8, 19, RectLayout.MAX, RectLayout.MAX)
		# Hardly gain anything from scaling this up
		gSetRect("CenterBottomPanel", "Top", gRect("LowerLeftCorner").xRight(), RectLayout.BOTTOM, -gRect("LowerRightCorner").width(), HLEN(133, 0.5))
		# Sans the decoration on top
		gSetRect("CenterBottom", "CenterBottomPanel", 0, 17, RectLayout.MAX, RectLayout.MAX)

		gSetRect("CityLeftPanel", "Top", 0, 0, HLEN(258), -gRect("LowerLeftCorner").height())
		...
		# Doesn't seem to hurt to just place the Left and Right panel (a bit) farther to the bottom, I guess because their upper edge overlaps with a different panel,
		...
		gSetPoint("GoldText", PointLayout((gRect("TurnLogButton").x() * 2) / 3, gPoint("TimeText").y()))
		gSetPoint("EraText", PointLayout(gRect("CityLeftPanel").xRight() - HSPACE(8), gPoint("GoldText").y()))
		...
		gSetSquare("EndTurnButton", "Top", -gRect("LowerRightCorner").width() + iEndTurnBtnSz / 2, -gRect("LowerRightCorner").height() + iEndTurnBtnSz / 2 + 2, iEndTurnBtnSz)
		gSetRect("CivFlagArea", "LowerRightCorner", 0, 0, gRect("MiniMapPanel").x() - gRect("LowerRightCorner").x() + 2, RectLayout.MAX)
		iFlagWidth = 68
		gSetRect("CivilizationFlag", "CivFlagArea", RectLayout.CENTER, 2 * (gRect("LowerRightCorner").height() - gRect("CenterBottom").height()) / 5, iFlagWidth, iFlagWidth * 250 / 68)
# BUG - City Arrows - start
		lMainCityScrollButtons = RowLayout(gRect("Top"), (gRect("EndTurnButton").xRight() + gRect("MiniMapPanel").x()) / 2, gRect("EndTurnButton").y(), 2, HSPACE(-12), BTNSZ(32))
...
# BUG - City Arrows - end
		...
		# The height of the bottom button list isn't known here, but the width is - and that width is needed in other places.
		# So let's define a helper rectangle with maximal height.
		iRMargin = gRect("CenterBottom").xRight() - gRect("CityTabs").x()
		iLMargin = iRMargin / 2
		iRMargin += 8
		iLMargin += 8
		gSetRect("BottomButtonMaxSpace", "CenterBottom", iLMargin, 0, -iRMargin, RectLayout.MAX)
...
# BUG - field of view slider - start
		...
		# (like K-Mod does) to avoid recursion - BUG stores the value computed for self.DEFAULT_FIELD_OF_VIEW in CvGlobals, overwriting the FIELD_OF_VIEW set through XML until the game is restarted.
...
# BUG - Progress Bar - Tick Marks - end
		# fExp consistent with the org. button heights
		gSetRect("CityOrgArea", "CityRightPanel", RectLayout.CENTER, gRect("CityAdjustPanel").y(), -HSPACE(9), VLEN(50, 0.5))
		...
		gSetRect("CityRightPanelContents", "CityRightPanel", RectLayout.CENTER, gRect("CityOrgArea").yBottom() + VSPACE(4), -HSPACE(9), gRect("LowerRightCornerPanel").y() - gRect("LowerRightCorner").y() + 4)
		gSetRect("GreatPeopleBar", "CityRightPanelContents", 0, RectLayout.BOTTOM, RectLayout.MAX, self.stackBarDefaultHeight())
		...
		lCultureBars = ColumnLayout(gRect("CityLeftPanelContents"), 0, RectLayout.BOTTOM, 2, VSPACE(-1), gRect("CityLeftPanelContents").width(), self.stackBarDefaultHeight())
		...
		self.m_iNumPlotListButtonsPerRow = ((gRect("BottomButtonMaxSpace").width() - 2 * iPlotListUnitBtnSz) / iPlotListUnitBtnSz)
		...
		# BtS leaves some space here, but we want the text area as low as possible to maximize the amount of text we can display (while also having tall corners
		...
		gSetRect("DefaultHelpArea", "Top", HSPACE(7), gRect("LowerLeftCornerBackgr").y(), max(280, gRect("LowerLeftCornerPanel").width() - HLEN(0.12 * gRect("LowerLeftCornerPanel").width())), 0)
	...
	def setMiniMapRects(self):
		...
		gSetRect("MiniMapPanel", "Top", -iMiniMapPanelRMargin, RectLayout.BOTTOM, iMiniMapPanelWidth, iMiniMapHeight + gRect("MiniMapButton").size() / 2 + 3 + 2 * iMiniMapVMargin)
...
# BUG - Great Person Bar, BUG - Great General Bar - end
	...
	def setAdvisorButtonRects(self):
		iSize = BTNSZ(28)
		advisorNames = [ "Domestic", "Finance", "Civics", "Foreign", "Military", "Tech", "Religious", "Corporation", "Victory", "Info" ]
	...
	def setCommerceAdjustRects(self):
		...
		for i in range(iMaxRows):
			gOffSetPoint("PercentText" + str(i), "Top", HSPACE(14), gRect("InterfaceTopLeft").yBottom() + VSPACE(-8) + i * iRowH)
			if self.bScaleHUD:
		...
		for i in range(iMaxRows):
			...
			gSetPoint("CityPercentText" + str(i), PointLayout(gRect("CommerceSliderBtns3").xRight() + HSPACE(70, 3), iY))
		...
		iVSpace = VSPACE(5)
		gSetRect("CityAdjustPanel", "CityLeftPanel", RectLayout.CENTER, gPoint("PercentText0").y() - gRect("CityLeftPanel").y() - iVSpace, min(-1, gRect("CityLeftPanel").x() - gPoint("PercentText0").x() + HSPACE(6)), gPoint("MaintenanceText").y() + iBtnSize + 1 - gPoint("PercentText0").y() + 2 * iVSpace)
	...
	def setTradeRouteRects(self):
		...
		gSetRect("TradeRouteTable", "CityLeftPanelContents", RectLayout.CENTER, 1 + gRect("TradeRouteListBackground").yBottom() - gRect("CityLeftPanelContents").y(), -1, 2 + iHeightForRows - iHeightForRows % iRowH)
# BUG - Raw Yields - start
...
# BUG - Raw Yields - end
	...
	def setBuildingListRects(self):
		...
		gSetRect("BuildingListTable", "CityLeftPanelContents", RectLayout.CENTER, 1 + gRect("BuildingListBackground").yBottom() - gRect("CityLeftPanelContents").y(), -1, gRect("CultureBars").y() - gRect("BuildingListBackground").yBottom() - 1 - VSPACE(4))
	...
	def setCityTaskRects(self):
		...
		gSetRect("CityTaskArea", "LowerRightCorner", 0, 0, gRect("MiniMapPanel").x() - gRect("LowerRightCorner").x(), RectLayout.MAX)
		...
		iLargeBtnW = 64
		iLargeBtnH = 30
		gSetRect("CityTaskBtns", "CityTaskArea", RectLayout.CENTER, RectLayout.CENTER, iLargeBtnW, iSmallBtnH + iHighBtnH + 2 * iMediumBtnH + iLargeBtnH)
		# The above doesn't look centered, due to widget styles I guess.
		gRect("CityTaskBtns").move(2, 3)
		gSetRect("Conscript", "CityTaskBtns", 0, 0, iLargeBtnW, iLargeBtnH)
		lHurryBtns = RowLayout(gRect("CityTaskBtns"), 0, gRect("Conscript").height(), 2, 0, iMediumBtnW, iMediumBtnH)
		for i in range(2):
			gSetRectangle("Hurry" + str(i), lHurryBtns.next())
		lAutomateBtns = RowLayout(gRect("CityTaskBtns"), 0, gRect("Hurry0").yBottom() - gRect("CityTaskBtns").y(), 2, 0, iMediumBtnW, iMediumBtnH)
		gSetRectangle("AutomateProduction", lAutomateBtns.next())
		gSetRectangle("AutomateCitizens", lAutomateBtns.next())
		lEmphYieldBtns = RowLayout(gRect("CityTaskBtns"), 0, gRect("AutomateProduction").yBottom() - gRect("CityTaskBtns").y(), 3, 0, iSmallBtnW, iSmallBtnH)
		for i in range(3):
			gSetRectangle("Emphasize" + str(i), lEmphYieldBtns.next())
		lEmphMiscBtns = RowLayout(gRect("CityTaskBtns"), 0, gRect("Emphasize0").yBottom() - gRect("CityTaskBtns").y(), 3, 0, iSmallBtnW, iHighBtnH)
		for i in range(3):
	...
	def setInfoPaneRects(self):
		...
		gOffSetPoint("SelectedUnitLabel", "SelectedUnitPanel", HSPACE(11), VSPACE(2))
...
# BUG - Stack Promotions - end
	...
	def setCitizenButtonRects(self):
		# (This will be a bit ugly because the stuff is right-aligned and because it's really a 2D grid. The RectLayout classes are
		...
		iNonAdjustablesOffset = (3 * iSmallVSpace + 2 * iCitizenBtnSize + gRect("GreatPeopleBar").height())
		...
		gRect("FirstFreeSpecialist").move(-gRect("AdjustSpecialistButtons").width() - iBigHSpace, -gRect("GreatPeopleBar").height() - iSmallVSpace)
		...
		lAngryCitizenButtons.move(-gRect("AdjustSpecialistButtons").width() - iBigHSpace, -gRect("GreatPeopleBar").height() - gRect("FirstFreeSpecialist").height() - 2 * iSmallVSpace)
...
# BUG - city specialist - end
	...
	def setCityBonusRects(self):
		...
		gSetRect("BonusPane0", "CityRightPanelContents", 0, 0, HLEN(53), -(gRect("CityRightPanelContents").yBottom() - (gRect("SpecialistLabelBackground").yBottom() - iBonusBackrOverhang)))
		...
		# because the plus signs are gone.
		gSetRect("BonusPane1", "CityRightPanelContents", gRect("BonusPane0").width(), 0, HLEN(78), gRect("BonusPane0").height())
		# width was 184
		...
		gSetRect("BonusPane2", "CityRightPanelContents", gRect("BonusPane0").width() + gRect("BonusPane1").width(), 0, RectLayout.MAX, gRect("BonusPane0").height())
		# width was 205
	...
	def interfaceScreen (self):
		...
		# Turn log Button
		self.setStyledButton("TurnLogButton", "Button_HUDLog_Style", WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_TURN_LOG).getActionInfoIndex())
		...
		self.setStyledButton("FinanceAdvisorButton", "Button_HUDAdvisorFinance_Style", WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_FINANCIAL_SCREEN).getActionInfoIndex())
		screen.hide("FinanceAdvisorButton")
		self.setStyledButton("CivicsAdvisorButton", "Button_HUDAdvisorCivics_Style", WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_CIVICS_SCREEN).getActionInfoIndex())
		screen.hide("CivicsAdvisorButton")
		self.setStyledButton("ForeignAdvisorButton", "Button_HUDAdvisorForeign_Style", WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_FOREIGN_SCREEN).getActionInfoIndex())
		...
		self.setStyledButton("ReligiousAdvisorButton", "Button_HUDAdvisorReligious_Style", WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_RELIGION_SCREEN).getActionInfoIndex())
		screen.hide("ReligiousAdvisorButton")
		self.setStyledButton("CorporationAdvisorButton", "Button_HUDAdvisorCorporation_Style", WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_CORPORATION_SCREEN).getActionInfoIndex())
...
# BUG - city specialist - end
		...
		screen.setStackedBarColors("TwoLineResearchBar", InfoBarTypes.INFOBAR_STORED, gc.getInfoTypeForString("COLOR_RESEARCH_STORED"))
		screen.setStackedBarColors("TwoLineResearchBar", InfoBarTypes.INFOBAR_RATE, gc.getInfoTypeForString("COLOR_RESEARCH_RATE"))
		screen.setStackedBarColors("TwoLineResearchBar", InfoBarTypes.INFOBAR_RATE_EXTRA, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.setStackedBarColors("TwoLineResearchBar", InfoBarTypes.INFOBAR_EMPTY, gc.getInfoTypeForString("COLOR_EMPTY"))
...
# BUG - Great General Bar - start
		...
		screen.setStackedBarColors("TwoLineGGBar", InfoBarTypes.INFOBAR_STORED, gc.getInfoTypeForString("COLOR_NEGATIVE_RATE"))
		screen.setStackedBarColors("TwoLineGGBar", InfoBarTypes.INFOBAR_RATE, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.setStackedBarColors("TwoLineGGBar", InfoBarTypes.INFOBAR_RATE_EXTRA, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.setStackedBarColors("TwoLineGGBar", InfoBarTypes.INFOBAR_EMPTY, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.hide("TwoLineGGBar")
# BUG - Great General Bar - end
# BUG - Great Person Bar - start
		self.addStackedBar("TwoLineGPBar", WidgetTypes.WIDGET_GP_PROGRESS_BAR)
		screen.setStackedBarColors("TwoLineGPBar", InfoBarTypes.INFOBAR_STORED, gc.getInfoTypeForString("COLOR_GREAT_PEOPLE_STORED"))
		screen.setStackedBarColors("TwoLineGPBar", InfoBarTypes.INFOBAR_RATE, gc.getInfoTypeForString("COLOR_GREAT_PEOPLE_RATE"))
		screen.setStackedBarColors("TwoLineGPBar", InfoBarTypes.INFOBAR_RATE_EXTRA, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.setStackedBarColors("TwoLineGPBar", InfoBarTypes.INFOBAR_EMPTY, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.hide("TwoLineGPBar")
# BUG - Great Person Bar - end
# BUG - Bars on single line for higher resolution screens - start
		self.addStackedBar("OneLineGGBar", WidgetTypes.WIDGET_HELP_GREAT_GENERAL)
		#gc.getInfoTypeForString("COLOR_GREAT_PEOPLE_STORED"))
		screen.setStackedBarColors("OneLineGGBar", InfoBarTypes.INFOBAR_STORED, gc.getInfoTypeForString("COLOR_NEGATIVE_RATE"))
		screen.setStackedBarColors("OneLineGGBar", InfoBarTypes.INFOBAR_RATE, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.setStackedBarColors("OneLineGGBar", InfoBarTypes.INFOBAR_RATE_EXTRA, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.setStackedBarColors("OneLineGGBar", InfoBarTypes.INFOBAR_EMPTY, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.hide("OneLineGGBar")

		self.addStackedBar("OneLineResearchBar", WidgetTypes.WIDGET_RESEARCH)
		screen.setStackedBarColors("OneLineResearchBar", InfoBarTypes.INFOBAR_STORED, gc.getInfoTypeForString("COLOR_RESEARCH_STORED"))
		screen.setStackedBarColors("OneLineResearchBar", InfoBarTypes.INFOBAR_RATE, gc.getInfoTypeForString("COLOR_RESEARCH_RATE"))
		screen.setStackedBarColors("OneLineResearchBar", InfoBarTypes.INFOBAR_RATE_EXTRA, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.setStackedBarColors("OneLineResearchBar", InfoBarTypes.INFOBAR_EMPTY, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.hide("OneLineResearchBar")

		self.addStackedBar("OneLineGPBar", WidgetTypes.WIDGET_GP_PROGRESS_BAR)
		screen.setStackedBarColors("OneLineGPBar", InfoBarTypes.INFOBAR_STORED, gc.getInfoTypeForString("COLOR_GREAT_PEOPLE_STORED"))
		screen.setStackedBarColors("OneLineGPBar", InfoBarTypes.INFOBAR_RATE, gc.getInfoTypeForString("COLOR_GREAT_PEOPLE_RATE"))
		screen.setStackedBarColors("OneLineGPBar", InfoBarTypes.INFOBAR_RATE_EXTRA, gc.getInfoTypeForString("COLOR_EMPTY"))
		screen.setStackedBarColors("OneLineGPBar", InfoBarTypes.INFOBAR_EMPTY, gc.getInfoTypeForString("COLOR_EMPTY"))
...
# BUG - PLE - begin
		...
		for j in range(iRows):
			...
			gSetRect(szPanelName, "Top", gRect("LowerLeftCornerPanel").xRight() + HSPACE(2) + 6, gRect("CenterBottomPanel").y() - VSPACE(2) - 4 + (j - iRows) * iBtnSize, iCols * iBtnSize + HSPACE(3), iBtnSize + VSPACE(1))
...
# BUG - NJAGC - start
		...
		if (CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START):
			...
			self.updateTimeText()
			self.setLabel("TimeText", "Background", g_szTimeText, CvUtil.FONT_RIGHT_JUSTIFY, FontTypes.GAME_FONT, -0.3)
...
# BUG - city specialist - end
	...
	def isShowSpecialistLabel(self):
		# Not quite enough space on low res. (Fixme: The Stacker layout does leave enough space, but then we'd need a separate label and background panel placed by updateCitizenButton_Stacker; currently, the position is set statically, ignoring options.)
...
# BUG - Min/Max Sliders - start
			...
			self.setStyledButton(szString, ButtonStyles.BUTTON_STYLE_CITY_PLUS, WidgetTypes.WIDGET_CHANGE_PERCENT, eCommerce, gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"))
			screen.show(szString)
			screen.enable(szString, bEnable)
			szString = "DecreasePercent" + str(eCommerce)
			gSetRectangle(szString, gRect("CommerceSliderBtns" + str(iCol + 1)).next())
			self.setStyledButton(szString, ButtonStyles.BUTTON_STYLE_CITY_MINUS, WidgetTypes.WIDGET_CHANGE_PERCENT, eCommerce, -gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"))
			screen.show(szString)
			screen.enable(szString, bEnable)
...
#		if not (pPlot and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyEngine().isGlobeviewUp() == False):
		if (not pPlot or CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_HIDE_ALL or CyEngine().isGlobeviewUp() == True):
			self.BupPanel.clearUnits()
			self.BupPanel.Hide()
			return 0
...
# BUG - BUG unit plot draw method - end
	...
	def updateSelectionButtons(self):
		...
		if not CyEngine().isGlobeviewUp() and pHeadSelectedCity:
			...
			if (pHeadSelectedCity.getOwner() == gc.getGame().getActivePlayer() or gc.getGame().isDebugMode()):
				...
				self.setStyledButton("Conscript", "Button_CityT1_Style", WidgetTypes.WIDGET_CONSCRIPT, -1, -1, None, "<font=1>" + localText.getText("TXT_KEY_DRAFT", ()) + "</font>")
				screen.hide("Conscript")
			...
			if CyInterface().getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION:
				...
				if (pHeadSelectedUnit and pHeadSelectedUnit.getOwner() == gc.getGame().getActivePlayer() and bHeadSelectionChanged):
					...
					for i in actions:
						...
						# When space is very limited, then there's a danger of about half of the units using a single (wrapped)
	...
	def appendGroupBottomButtons(self):
		...
		if CyInterface().canCreateGroup():
			self.screen.appendMultiListButton("BottomButtonList", ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_CREATEGROUP").getPath(), 0, WidgetTypes.WIDGET_CREATE_GROUP, -1, -1, False)
			iAppended += 1
		if CyInterface().canDeleteGroup():
			self.screen.appendMultiListButton("BottomButtonList", ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_SPLITGROUP").getPath(), 0, WidgetTypes.WIDGET_DELETE_GROUP, -1, -1, False)
			iAppended += 1
		return iAppended
	...
	def updateUnitCyclingButtons(self, pHeadSelectedUnit):
		if MainOpt.isUnitCyclingButtonsDisabled():
			return
		lButtons = ColumnLayout(gRect("CenterBottom"), -HSPACE(5), VSPACE(5), 2, VSPACE(4), BTNSZ(32))
...
# BUG - city specialist - start
	def updateCitizenButtons_Stacker(self, pHeadSelectedCity):
		...
		if iFreeSpecialists > 0:
			...
			# advc.001: Don't re-use the BtS name because that leads to problems
		...
		if gc.getPlayer(ePlayer).isAlive():
			...
			for i in range(CommerceTypes.NUM_COMMERCE_TYPES):
				...
				if (not CyInterface().isCityScreenUp() and (eCommerce != CommerceTypes.COMMERCE_CULTURE or MainOpt.isShowTotalCultureRate())):
					szOutText = u"<font=2>" + localText.getText("TXT_KEY_MISC_POS_GOLD_PER_TURN", (gc.getPlayer(ePlayer).getCommerceRate(CommerceTypes(eCommerce)),))
					szOutText += u"</font>"
...
# BUG - Gold Rate Warning - end
		self.setLabel("GoldText", "Background", szText, CvUtil.FONT_LEFT_JUSTIFY, FontTypes.GAME_FONT, -0.3)
...
# BUG - NJAGC - start
		...
		if ClockOpt.isShowEra():
			...
			self.setLabel("EraText", "Background", szText, CvUtil.FONT_RIGHT_JUSTIFY, FontTypes.GAME_FONT, -0.3)
			screen.show("EraText")
# BUG - NJAGC - end
# BUG - Bars on single line for higher resolution screens - start
		if self.isShowTopBarsOnSingleLine():
			...
			szResearchBar = "TwoLineResearchBar"
		gOffSetPoint("ResearchText", szResearchBar, RectLayout.CENTER, self.stackBarDefaultTextOffset())
...
# BUG - Great Person Bar - start
	def updateGreatPersonBar(self, screen):
		...
		if pGPCity:
			...
			iCityID = -1
# BUG - Bars on single line for higher resolution screens - start
		self.setText("GreatPersonBarText", "Background", szText, CvUtil.FONT_CENTER_JUSTIFY, FontTypes.GAME_FONT, -0.4, WidgetTypes.WIDGET_GP_PROGRESS_BAR)
		if not pGPCity:
...
# BUG - Raw Commerce - end
		...
		screen.setHelpTextArea(HLEN(390), FontTypes.SMALL_FONT, 0, 0, -2.2, True, ArtFileMgr.getInterfaceArtInfo("POPUPS_BACKGROUND_TRANSPARENT").getPath(), True, True, CvUtil.FONT_LEFT_JUSTIFY, 0)
...
# BUG - Food Assist - end
			...
			self.setLabel("PopulationText", "Background", szBuffer, CvUtil.FONT_CENTER_JUSTIFY, FontTypes.GAME_FONT, -1.3)
...
# BUG - Food Rate Hover - start
		...
		self.setLabel("PopulationInputText", "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, FontTypes.GAME_FONT, -0.3, WidgetTypes.WIDGET_FOOD_MOD_HELP)
...
# BUG - Food Rate Hover - end
		if (pHeadSelectedCity.badHealth(False) > 0 or pHeadSelectedCity.goodHealth() >= 0):
			...
			self.setLabel("HealthText", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, FontTypes.GAME_FONT, -0.3, WidgetTypes.WIDGET_HELP_HEALTH)
...
# BUG - Whip Assist - end
			self.setLabel("ProductionText", "Background", szBuffer, CvUtil.FONT_CENTER_JUSTIFY, FontTypes.GAME_FONT, -1.3, WidgetTypes.WIDGET_GENERAL)
		...
		self.setLabel("ProductionInputText", "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, FontTypes.GAME_FONT, -0.3, WidgetTypes.WIDGET_PRODUCTION_MOD_HELP)
...
# BUG - Anger Display - end
			self.setLabel("HappinessText", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, FontTypes.GAME_FONT, -0.3, WidgetTypes.WIDGET_HELP_HAPPINESS)
...
# BUG - Progress Bar - Tick Marks - end
		...
		for i in range(CommerceTypes.NUM_COMMERCE_TYPES):
			...
			if self.showCommercePercent(eCommerce, pHeadSelectedCity.getOwner()): # K-Mod
				...
				self.setLabel(szName, "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, FontTypes.SMALL_FONT, -0.3, WidgetTypes.WIDGET_COMMERCE_MOD_HELP, eCommerce)
				screen.show(szName)
				iCount += 1
		# <advc.092>
...
# BUG - Raw Yields - end
		...
		# Something chronological based on tech reqs (but also taking special care of wonders and free buildings) should work better than the XML ordering
		# - tbd.?
		'''
		...
		for iBuilding in aCityBldgs:
			for k in range(pHeadSelectedCity.getNumBuilding(iBuilding)):
				...
				screen.setTableText("BuildingListTable", 0, iNumBuildings, szFontStart + szLeftBuffer + "</font>", szIcon, WidgetTypes.WIDGET_HELP_BUILDING, iBuilding, -1, CvUtil.FONT_LEFT_JUSTIFY)
				screen.setTableText("BuildingListTable", 1, iNumBuildings, szFontStart + szRightBuffer + "</font>", "", WidgetTypes.WIDGET_HELP_BUILDING, iBuilding, -1, CvUtil.FONT_RIGHT_JUSTIFY)
...
# K-Mod: Trade culture end
				if not bShowRawYields:
					...
					screen.setTableText("TradeRouteTable", 0, iNumTradeRoutes, szFontStart + szLeftBuffer + "</font>", "", WidgetTypes.WIDGET_HELP_TRADE_ROUTE_CITY, i, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.setTableText("TradeRouteTable", 1, iNumTradeRoutes, szFontStart + szRightBuffer + "</font>", "", WidgetTypes.WIDGET_HELP_TRADE_ROUTE_CITY, i, -1, CvUtil.FONT_RIGHT_JUSTIFY)
# BUG - Raw Yields - end
		...
		for iBonus in aCityBonuses:
			...
			if iHappiness != 0:
				if iHappiness > 0:
					szTempBuffer = szFontStart + (u"%d%c</font>" %(iHappiness, CyGame().getSymbolID(FontSymbols.HAPPY_CHAR)))
				else:
					szTempBuffer = szFontStart + (u"%d%c</font>" %(-iHappiness, CyGame().getSymbolID(FontSymbols.UNHAPPY_CHAR)))
				if iHealth > 0:
					szTempBuffer += szFontStart + (u" %d%c</font>" %(iHealth, CyGame().getSymbolID(FontSymbols.HEALTHY_CHAR)))
				# <advc.092>
				if self.bCityBonusButtons:
			...
			if iHealth != 0 and bHandled == False:
				if iHealth > 0:
					...
					szTempBuffer = szFontStart + (u"%d%c</font>" %(-iHealth, CyGame().getSymbolID(FontSymbols.UNHEALTHY_CHAR)))
				# <advc.092>
				if self.bCityBonusButtons:
		...
		self.setLabel("MaintenanceText", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, FontTypes.SMALL_FONT, -0.3, WidgetTypes.WIDGET_HELP_MAINTENANCE)
		...
		self.setLabel("MaintenanceAmountText", "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, FontTypes.SMALL_FONT, -0.3, WidgetTypes.WIDGET_HELP_MAINTENANCE)
...
# BUG - Limit/Extra Religions - start
		if CityScreenOpt.isShowOnlyPresentReligions():
			...
			aReligionRows = self.cityOrgRects(len(aReligions), gc.getNumReligionInfos())
			...
			for i in range(len(aReligions)):
				...
				# because that string is never used. (Already unused in Vanilla Civ 4.)
				'''
			...
			lReligions = RowLayout(gRect("CityOrgArea"), RectLayout.CENTER, 0, gc.getNumReligionInfos(), HSPACE(10), BTNSZ(24, 0.5))
			gSetRectangle("CityReligions", lReligions)
			for iReligion in range(gc.getNumReligionInfos()):
				lReligion = lReligions.next()
				bEnable = True
...
# BUG - Limit/Extra Corporations - start
		...
		if CityScreenOpt.isShowOnlyPresentReligions():
			...
			for iCorp in range(gc.getNumCorporationInfos()):
				...
				aCorporations += [iCorp]
			# <advc.092>
			aCorpRows = self.cityOrgRects(len(aCorporations), gc.getNumCorporationInfos())
			...
			lCorporations = RowLayout(gRect("CityOrgArea"), RectLayout.CENTER, gRect("CityReligions").height() + iReligionCorpsMargin, gc.getNumReligionInfos(), HSPACE(10), BTNSZ(24, 0.5))
			for iCorp in range(gc.getNumCorporationInfos()):
...
# BUG - Limit/Extra Corporations - end
		...
		self.setLabel("NationalityText", "Background", szBuffer, CvUtil.FONT_CENTER_JUSTIFY, FontTypes.SMALL_FONT, -0.3)
		...
		if (iDefenseModifier != 0):
			...
			self.setLabel("DefenseText", "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, FontTypes.SMALL_FONT, -0.3, WidgetTypes.WIDGET_HELP_DEFENSE)
...
# BUG - Culture Turns - end
			self.setLabel("CultureText", "Background", szBuffer, CvUtil.FONT_CENTER_JUSTIFY, FontTypes.GAME_FONT, -1.3)
...
# BUG - Great Person Turns - end
			self.setLabel("GreatPeopleText", "Background", szBuffer, CvUtil.FONT_CENTER_JUSTIFY, FontTypes.GAME_FONT, -1.3)
	...
	def fillCityBonusRow(self, iColumn, iRow, iBonus, iAmount, szEffect = None):
		...
		if iAmount > 1:
			...
			szAmount = (u"<font=2>" + localText.changeTextColor(str(iAmount), gc.getInfoTypeForString("COLOR_YELLOW")) + "</font>")
...
# BUG - Align Icons - end
	...
	def updateScoreBackgrSize(self, iTextWidth, iTextHeight):
		...
		self.screen.setPanelSize("ScoreBackground", gRect("Top").xRight() - iTextWidth - 2 * iRMargin, gRect("GlobeToggle").y() - iTextHeight - (5 * iBMargin) / 2, iTextWidth + (7 * iRMargin) / 4, iTextHeight + 3 * iBMargin)
	...
	def updateScoreStrings(self, bOnlyBackgr = False): # advc.004z: new param
		...
		# </advc.092>
		gSetPoint("ScoreTextLowerRight", PointLayout(gRect("MiniMap").xRight(), gRect("GlobeToggle").y() - VSPACE(6)))
# BUG - Align Icons - start
		...
		if pPlayer.isAlive():
			...
			# K-Mod end
				if pTeam.isAtWar(eActiveTeam):
					szBuffer += "("
					szBuffer += localText.getColorText("TXT_KEY_CONCEPT_WAR", (), gc.getInfoTypeForString("COLOR_RED")).upper()
					szBuffer += ")"
					if bAlignIcons:
						scores.setWar()
...
# BUG - Stack Promotions - end
	...
	def cityOrgRects(self, iOrgs, iMaxOrgs):
		...
		lMaxOrgs = RowLayout(gRect("CityOrgArea"), RectLayout.CENTER, 0, iMaxOrgs, iButtonSpace, iButtonSize)
		...
		while iOrgs > 0:
			...
			aResult.append(RowLayout(gRect("CityOrgArea"), iLMargin, iTopMargin, iButtonsPerRow, iButtonSpace, iButtonSize))
...
# BUG - Great Person Bar - start
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED and inputClass.getFunctionName().startswith("TwoLineGPBar")):
