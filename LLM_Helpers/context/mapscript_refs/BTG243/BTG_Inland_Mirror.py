...
def getDescriptionTitle():
	return "An inner see in the middle of a lot of arable land, one of the map with the most rivers and flooplains."

def getDescriptionTitleTwo():
	return ""

def getDescriptionMain():
	return "This is the mirror version along the Y axis, make sure you play with 2 teams, otherwise starting positions will break"

def getDescriptionSecond():#Script tip : (on TOP)
	return "Play with low sea to give a change to have possibilities to have land with no culture in between players."	
	
def getDescriptionThird():#Option : (at the bottom)"
	return ""	
	
def getDescriptionScenario():#Scenario : (at the bottom)"
	return "Best suited for 3v3 Medieval and Renaissance Start. Also the most balanced map for 1v1 duel and 1v1 Ironman"	

def getDescriptionBalance():#Balance : (at the bottom)"
	return "AGRICULTURAL is very strong on this map with lots of floodplains yielding 5F."		

resourcesToBalance = ('BONUS_IRON', 'BONUS_HORSE', 'BONUS_COPPER','BONUS_OIL')
newFlavorResource = ('BONUS_TEA','BONUS_OLIVES','BONUS_SALT')
newStrategicResource = ('BONUS_AMBER','BONUS_SULPHUR','BONUS_LEAD','BONUS_NICKEL','BONUS_DIAMOND','BONUS_JADE','BONUS_SAPPHIRES','BONUS_OBSIDIAN', 'BONUS_POTATO')
...
def getCustomMapOptionDescAt(argsList):
	...
	selection_names = {	
		0:	{
			0: "Normal - Vertical Symmetry",
			1: "Inversed - Central Symmetry"
			},				
		1:	{
			0: "Normal Sizes",
			1: "1 Notch Bigger (useful for Duels on vertical symmetry)"
			},
		2:	{
			0: "No - No food nor Hill normalization (weaker Land)",
			1: "Yes - Default C++ Normalization",
			2: "Yes - Penny Deer Normalization",
			3: "Yes - Normalization and Deer Always"
			},				
		3:	{
			0: "No",
			1: "Yes - BTG Resources once by side"
			},				
		4:	{
			0: "Normal Forest",
			1: "75% Forest - 25% Palm",
		...
		6:	{
			0: "Starting Position Only for 2 teams"
			},
		7:	{
			0: "Team - 1 Spectator good in FIRST slot"			
			},				
		8:	{
			0: "Based on FM Mirror Inland Sea by Shizanu",
			1: "Penny for BTG - Works for BTS, BTG Options have no effect"	
			}				
		}
...
def beforeInitOptionsValue():#2.36 this is handy to find in code
	...
	global iOptionValue_MirrorType
	global iOptionValue_Size
	global iOptionValue_Normalization
	global iOptionValue_ResourceBTG
	global iOptionValue_ForestType
	global iOptionValue_SameTile

	iOptionValue_MirrorType = CyMap().getCustomMapOption(0)
	iOptionValue_Size = CyMap().getCustomMapOption(1)
	iOptionValue_Normalization = CyMap().getCustomMapOption(2)
	iOptionValue_ResourceBTG = CyMap().getCustomMapOption(3)
	iOptionValue_ForestType = CyMap().getCustomMapOption(4)
	iOptionValue_SameTile = CyMap().getCustomMapOption(5)
...
def generateTerrainTypes():
	...
	if iOptionValue_MirrorType == 1:#Inversed Type
...
def assignStartingPlots():
	...
	#2.15 - not enough
	if isBTPon:
		#if gc.getTeam(2).isSpectator():
		if gc.getTeam(0).isSpectator():#2.22
			team_one = gc.getTeam(1)
			team_two = gc.getTeam(2)
		else:
	...
	#2.15 - not enough
	'''if gc.getPlayer(0).isSpectator():
		for iLoop in range(1,iPlayers+1):
			thisPlayerID = player_list[iLoop]
			this_player = gc.getPlayer(thisPlayerID)
			teamID = gc.getPlayer(thisPlayerID).getTeam()
			print("Player: ", thisPlayerID, " Team: ", teamID)
			if teamID == 2:
				playersOnTeamTwo.append(player_list[iLoop])
			else:
				playersOnTeamOne.append(player_list[iLoop])
	...
	userInputPlots = iOptionValue_MirrorType #Inversed Type -- 2.38 Penny added this rule
...
def findStartingPlot(argsList):
	...
	def isValid(playerID, x, y):
		...
		numTeams = CyGlobalContext().getGame().countCivTeamsEverAlive()#BTP 2.18
...
def normalizeStartingPlotLocations():
	#numTeams = CyGlobalContext().getGame().countCivTeamsAlive() 
	numTeams = CyGlobalContext().getGame().countCivTeamsEverAlive()#BTP2.18
	userInputProximity = 0
...
def normalizeAddLakes():
	...
	#CyPythonMgr().allowDefaultImpl()
	return 0 # 2.39, it doesn't mirror well
...
def mirrorMap():
	...
	#2.18 -- Need to decide this before it's being mirrored
	if isBTPon:	
...
def normalizeAddExtras():
				...
				for bonus in range(gc.getNumBonusInfos()):
					...
					if (type_string not in resources_placed) and (type_string in resourcesToBalance):
						for (pLoopPlot) in plots:
							...
							if (pLoopPlot.canHaveBonus(bonus, True) and not pLoopPlot.isWater()):#2.36 Remove Oil water, always land
	...
	#### 2.36 #####
	if isBTPon and (iOptionValue_ResourceBTG > 0):#quicker if don't run when 0			
		for i in newStrategicResource:
			CvMapGeneratorUtil.BTPMapUtil().BTPResourceInBox(CyMap().getGridWidth() * 0 / 100,CyMap().getGridWidth() * 45 / 100, CyMap().getGridHeight()  * 0 / 100, CyMap().getGridHeight()  * 100 / 100, False, CyGlobalContext().getInfoTypeForString(i),1,0,-1)	
			CvMapGeneratorUtil.BTPMapUtil().BTPResourceInBox(CyMap().getGridWidth() * 55 / 100,CyMap().getGridWidth() * 100 / 100, CyMap().getGridHeight()  * 0 / 100, CyMap().getGridHeight()  * 100 / 100, False, CyGlobalContext().getInfoTypeForString(i),1,0,-1)			
	...
	if iOptionValue_Normalization >= 2:
		...
		BTGnormalizeAddExtrasFoodStart(iOptionValue_Normalization == 3,False,3,'BONUS_DEER',1)
	
	# map mirrored again, just to make sure
	mirrorMap()		

	if iOptionValue_Normalization == 1:
		CyPythonMgr().allowDefaultImpl()	# do the rest of the usual normalizeStartingPlots stuff, don't overrride
	else:
		return 0# 2.39 rewrite this, either none or Penny optiondone above
...
def BTGnormalizeAddExtrasFoodStart(bForcePlacement,bAllowedOnHills,iThresholdFood,iBonusType,iForestForceType):
...
def addBonusType(argsList):
	...
	#2.36 Rewrites this
	if isBTPon:
		if (type_string in newStrategicResource):
			return None
...
def startHumansOnSameTile(): return iOptionValue_SameTile