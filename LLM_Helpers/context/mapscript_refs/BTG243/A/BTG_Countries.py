...
def getCustomMapOptionName(argsList):
	...
	option_names = {
		...
		2:	"Country Type",		
		3:	"TXT_KEY_MAP_STARTING_SHAPE",
		4:  "Central Sea",
		5:	"Central Mountain",	
		6:	"TXT_MAP_NEW_RESOURCE",		
		7:	"TXT_KEY_FOREST_TYPE",
		8:	"TXT_KEY_MAP_SAME_TILE",
...
def getCustomMapOptionDescAt(argsList):
	...
	selection_names = {
		...
		3:	{
			0: "TXT_KEY_START_ANY_TEMPLATE",
			1: "TXT_KEY_START_V_SHAPE",
			2: "TXT_KEY_START_L_SHAPE",
			3: "TXT_KEY_START_ANYWHERE_TVBTHREE"
			},		
		4:	{
			0: "Water",
			1: "Snow",
		...
		6:	{
			...
			2: "TXT_KEY_MAP_YES_FIXED_LOCATION"
			},				
		7:	{
			0: "TXT_KEY_FOREST_TYPE_ALL_FOREST",
			1: "TXT_KEY_FOREST_TYPE_PALM_25",
			2: "TXT_KEY_FOREST_TYPE_PALM_50",
			3: "TXT_KEY_FOREST_TYPE_PALM_75",
			4: "TXT_KEY_FOREST_TYPE_ALL_PALM"		
			},	
		8:	{
		...
		9:	{
			...
			1: "TXT_KEY_MAP_SPECTATOR_TEAM_ONLY_TOP_ONE"			
			},
		10:	{
...
def addBonusType(argsList):
	...
	#2.21y
	if isBTPon:
		if not CyGlobalContext().getGame().isOption(GameOptionTypes.GAMEOPTION_NEW_STRATEGIC_RESOURCE) or (CyMap().getCustomMapOption(6) == 1):#all excluded
...
def normalizeStartingPlotLocations():
	...
	if (isBTPon and CyMap().getCustomMapOption(3) == 3):
		CvMapGeneratorUtil.BTPMapUtil().BTPTopBottomTwoTeams()			
	else:
...
def doCountry():#2.28 for Map Countries		
	...
	if (CyMap().getCustomMapOption(2) == 0):
		doRedesignColor(100,True,CyGlobalContext().getInfoTypeForString("TERRAIN_GRASS"),CyGlobalContext().getInfoTypeForString("TERRAIN_TUNDRA"), 30, CyGlobalContext().getInfoTypeForString("BONUS_DEER"), 10, CyGlobalContext().getInfoTypeForString("BONUS_FUR"))

		doChangeMiddle('Canada')
		
		
	if (CyMap().getCustomMapOption(2) == 1):
		doRedesignColor(100,True,CyGlobalContext().getInfoTypeForString("TERRAIN_PLAINS"),CyGlobalContext().getInfoTypeForString("TERRAIN_GRASS"), 45, CyGlobalContext().getInfoTypeForString("BONUS_SUGAR"), 10, CyGlobalContext().getInfoTypeForString("BONUS_SPICES"))
...
def doChangeMiddle(sCountry):
	if isBTPon:			
		...
		#Contition (3) depends on country you chose
			...
			if (CyMap().getCustomMapOption(4) == 1 or sCountry == 'Canada'):
...
def doRedesignColor(iProbaChange,bBonusAvoid, iTerrainChange,iTerrainNew,iProbaBonOne,iBonusOne,iProbaBonTwo,iBonusTwo):#2.28 for Map Countries			
