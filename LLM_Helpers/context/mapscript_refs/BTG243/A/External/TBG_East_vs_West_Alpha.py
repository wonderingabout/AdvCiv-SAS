...
class TeamBGTerrainGenerator(CvMapGeneratorUtil.TerrainGenerator):
	def getLatitudeAtPlot(self, iX, iY):
		# Latitudes run vertically for a world with a tilted axis. This copied from Tilted Axis
	...
	def generateTerrainAtPlot(self, iX, iY):
		...
		if lat >= self.fSnowLatitude:
			...
			#changed desertVal = self.deserts.getHeight(iX, iY)
			#changed plainsVal = self.plains.getHeight(iX, iY)
			desertVal = self.deserts.getHeight(iY, iX)
			plainsVal = self.plains.getHeight(iY, iX)
...
class TeamBGFeatureGenerator(CvMapGeneratorUtil.FeatureGenerator):
	def getLatitudeAtPlot(self, iX, iY):
		...
		#changed return 0.8 * (abs((self.iGridH/2) - iY)/float(self.iGridH/2))
		return 0.8 * (abs((self.iGridH/2) - iX)/float(self.iGridH/2))
