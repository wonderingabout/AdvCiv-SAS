	...
	templates = {(1,0): {0: [0.5, 0.5, int(0.5 * iW), int(0.5 * iH)]},
			...
			#Template for left-right setup
	             (6,0): {0: [0.1, 0.5, 0, 0],
	                     ...
	                     4: [0.7, 0.833, 0, 0],
	                     5: [0.3, 0.833, 0, 0]},
			#Template for top-bottom setup - replaced by (6,0)
	             (6,1): {0: [0.1, 0.5, 0, 0],
	                     1: [0.3, 0.167, 0, 0],
	                     2: [0.7, 0.167, 0, 0],
	                     3: [0.9, 0.5, 0, 0],
...
def findStartingPlot(argsList):
	...
	def isValid(playerID, x, y):
		...
		#Ergänzt um Überprüfung der Nähe zur mirrorline
		if x>13:
			if x<27:
				return false
...
def assignStartingPlots():
	...
	def isValidForMirror(playerID, x, y):
		...
		#change values here, to decide how close players start to the mirrorline

		# Two Evenly-Matched Teams, Start Together
		if iPlayers > 2 and userInputPlots <= 1 and x <= iW * 0.2:
			return true
		if iPlayers > 2 and userInputPlots >= 2 and x >= iW * 0.1 and x <= iW * 0.2:
			return true
