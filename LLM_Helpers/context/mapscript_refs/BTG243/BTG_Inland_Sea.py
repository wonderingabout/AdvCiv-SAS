...
def getDescriptionThird():#Option : (at the bottom)"
	return "The Central Land option let's you put some frozen tiles in the middle so that you can cross with galleys and triremes."	
	
def getDescriptionScenario():#Scenario : (at the bottom)"
	return "Best suited for 3v3 Medieval and Renaissance Start. Better balanced for 8 players CTON. If you play 6 players CTON, play with the 'L' configuration"
...
def getCustomMapOptionDescAt(argsList):
	...
	selection_names = {
		...
		3:	{
			...
			1: "Shallow Waters",
			2: "Snowy Ocean" ,
			3: "Frozen Artic Ocean",
			},	
		4:	{
			0: "No",
			1: "Yes - Balanced for Lead, Strategic for Jade",
			2: "Yes - Balanced as above, and 1 Sulphur in Middle"
			},				
		5:	{
			0: "Normal Forest",
			1: "75% Forest - 25% Palm",
...
def beforeInit():#2.38
	...
	global iOptionValue_StartingPosition
	global iOptionValue_StartingUnit
	global iOptionValue_CentralLand
	
	global iOptionValue_BTGResources
	global iOptionValue_BTGForest
	
	
	iOptionValue_Resources = CyMap().getCustomMapOption(0)
	iOptionValue_StartingPosition = CyMap().getCustomMapOption(1)
	iOptionValue_StartingUnit = CyMap().getCustomMapOption(2)	
	iOptionValue_CentralLand = CyMap().getCustomMapOption(3)
	
	iOptionValue_BTGResources = CyMap().getCustomMapOption(4)
	iOptionValue_BTGForest = CyMap().getCustomMapOption(5)
...
def normalizeAddExtras():
	if (iOptionValue_Resources == 1 or iOptionValue_Resources == 2):
		balancer.normalizeAddExtras()
		
		
	if (iOptionValue_CentralLand == 1):
		BTPFreezeOcean(True,False,100,False)	
	if (iOptionValue_CentralLand == 2):
		BTPFreezeOcean(False,False,22,False)		
	if (iOptionValue_CentralLand == 3):
		BTPFreezeOcean(False,True,22,False)			
		
	#2.18 Moved here, last thing to do
	if isBTPon:

		if (iOptionValue_BTGResources == 2):
		...
		if (iOptionValue_BTGResources >= 1):#2.35
			listToBalance = ["BONUS_LEAD"]
			balancer.BTPnormalizeAddExtrasSpecific(listToBalance,6,4,100)	

			CvMapGeneratorUtil.BTPMapUtil().BTPResourceInBox(CyMap().getGridWidth() * 0  / 100,CyMap().getGridWidth() * 30  / 100, CyMap().getGridHeight()  * 0  / 100, CyMap().getGridHeight()  * 30  / 100, False, CyGlobalContext().getInfoTypeForString("BONUS_JADE"),1,0,-1)
			CvMapGeneratorUtil.BTPMapUtil().BTPResourceInBox(CyMap().getGridWidth() * 0  / 100,CyMap().getGridWidth() * 30  / 100, CyMap().getGridHeight()  * 70 / 100, CyMap().getGridHeight()  * 100 / 100, False, CyGlobalContext().getInfoTypeForString("BONUS_JADE"),1,0,-1)
			CvMapGeneratorUtil.BTPMapUtil().BTPResourceInBox(CyMap().getGridWidth() * 70 / 100,CyMap().getGridWidth() * 100 / 100, CyMap().getGridHeight()  * 0  / 100, CyMap().getGridHeight()  * 30  / 100, False, CyGlobalContext().getInfoTypeForString("BONUS_JADE"),1,0,-1)
			CvMapGeneratorUtil.BTPMapUtil().BTPResourceInBox(CyMap().getGridWidth() * 70 / 100,CyMap().getGridWidth() * 100 / 100, CyMap().getGridHeight()  * 70 / 100, CyMap().getGridHeight()  * 100 / 100, False, CyGlobalContext().getInfoTypeForString("BONUS_JADE"),1,0,-1)
...
def addBonusType(argsList):
	...
	#2.21y
	if isBTPon:
		if (iOptionValue_BTGResources == 0):#all excluded
			if (type_string in balancer.newResourcesBTP):
				return None	
		if (iOptionValue_BTGResources == 1 or iOptionValue_BTGResources == 2):#all excluded
...
def beforeGeneration():
	...
	#2.15
	if (iOptionValue_StartingPosition == 1):
		iTemplateRoll = 0#overrides
	if (iOptionValue_StartingPosition == 2 or iOptionValue_StartingPosition == 3):
...
def normalizeStartingPlotLocations():
	...
	if (iOptionValue_StartingPosition == 3):
		if isBTPon :
...
def startHumansOnSameTile():
	...
	if isBTPon:
		if (iOptionValue_BTGForest > 0):#quicker if don't run when 0
			iProbaTreshold = 25 * iOptionValue_BTGForest #Gives %age on 100
			CvMapGeneratorUtil.BTPMapUtil().BTPforestIntoPalms(iProbaTreshold)


	if (iOptionValue_StartingUnit == 1):	return True	
	
		
		
		
def BTPFreezeOcean(bShallow,bFrozen,iProbaSnow,bAlsoCoast):
