...
def getDescriptionMain():
	return "The very center of the map has some impassable land to decrease the amount of initial conflict"
...
def getCustomMapOptionDescAt(argsList):
	...
	selection_names = {
		...
		8:	{
			0: "No",
			1: "Yes - Balanced for Lead, Middle 1 Sulphur 1 Amber"
			},				
		9:	{
			0: "Normal",
		...
		10:	{
			0: "4v4 on Standard, or 3v3 Standard, Max 5v5",
			1: "Each Size decrease changes 1 notch (6,7,8,9,10,11 Square size)",
			2: "The option with the bridge will give more land and 'feel' 1 notch larger"
			},			
		11:	{
...
def getWrapX():
	map = CyMap()
	return (map.getCustomMapOption(6) == 1 or map.getCustomMapOption(6) == 2)	
	...
	for x in range(CyMap().getGridWidth()):
		for y in range(CyMap().getGridHeight()):
			if (x == 0 or y == 0 or x == CyMap().getGridWidth()-1 or y == CyMap().getGridHeight()-1):
				if not (CyMap().getCustomMapOption(6) == 1 or CyMap().getCustomMapOption(6) == 2):#it's not nice when there is cylindrical wrap
					p = CyMap().plot(x,y)
...
def startHumansOnSameTile():
	...
	if isBTPon:#2.25
		if (CyMap().getCustomMapOption(7) > 0):
			...
			if (CyMap().getCustomMapOption(7)  == 3):
...
def BTGSong():
	...
	if (CyMap().getCustomMapOption(10) > 0):
		CyGame().setMapTriggerSound(CyMap().getCustomMapOption(10))
