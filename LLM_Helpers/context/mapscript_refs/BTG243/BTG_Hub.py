...
def getDescriptionTitle():
	return "A very popular BTS map which has the shape of a wheel, where each player gets an inner band of land which link all players to a central land"
...
def getDescriptionSecond():#Script tip : (on TOP)
	return "You to consider making the Spoke width 2 so that you can make the boats move more easily. "	
	
def getDescriptionThird():#Option : (at the bottom)"
	return "You might want to increase the amount of marshes to make it possible to cross without culture or cities"	
	
def getDescriptionScenario():#Scenario : (at the bottom)"
	return "Decent map for Ironman, very passive as you're only meeting in the middle, your island is very very safe. You will want to pay attention to the settings of the middle land."	
...
class HubMultilayeredFractal(CvMapGeneratorUtil.MultilayeredFractal):
	...
	def generateCenter(self, iWaterPercent, 
		...
		if (CyMap().getCustomMapOption(7) == 0):#the original line
			iHillsBottom1 = regionHillsFrac.getHeightFromPercent(max((10 - self.gc.getClimateInfo(self.map.getClimate()).getHillRange()), 0))
			...
			iHillsTop2 = regionHillsFrac.getHeightFromPercent(min((90 + self.gc.getClimateInfo(self.map.getClimate()).getHillRange()), 100))	
