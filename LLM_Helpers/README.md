...
- Emits combined RNG/state ordering interpretations only when both records expose the same explicit `recordRevision`. Missing or differing revision metadata leaves raw checkpoint comparisons available but suppresses recipe-sensitive combined conclusions.
