...
For example, [`Fix found during SASGameRecord revision 112: inherited AdvCiv AI deal-cancellation memory direction (KI#605.2)`](https://github.com/wonderingabout/AdvCiv-SAS/commit/003b9edd1fea0afa75b45846467ce7e8307c498b) showed that corrected diplomacy memory was the first state difference on turn 98 while the synchronized RNG remained identical through turn 101 before downstream history diverged on turn 102.

The later [KI#895](/_1_AdvCiv-SAS/Docs/README_Known_Issues.md#ki-895) / [KI#896](/_1_AdvCiv-SAS/Docs/README_Known_Issues.md#ki-896) `CvCity::kill` crash repair provides the complementary case: matching CORE/RNG provenance and event history confirmed that the fixed run had not diverged through the exact former capital-capture crash path before continuing safely afterward. `TODO: add the direct KI#895/KI#896 crash-fix commit link in a later commit`.
