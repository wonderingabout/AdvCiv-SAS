...
Another significant part of the project is [historical source archaeology](/README.md#historical-source-archaeology): instead of reviewing only the current source, historical BtS, K-Mod, AdvCiv and AdvCiv-SAS changes are followed through their actual commit diffs and lineage to find inherited bugs, regressions and suspicious logic that can survive for years unnoticed. The completed primary investigation currently records 131 confirmed candidate bug/issue findings (43 inherited/pre-SAS and 88 SAS-specific); fixes are implemented incrementally rather than delaying normal releases until every deferred finding is repaired.
