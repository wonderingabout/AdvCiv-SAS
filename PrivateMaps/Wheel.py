...
def beforeGeneration():
	...
	iPlayersActual = gc.getGame().countCivPlayersEverAlive()
	...
	if iPlayersActual < 2:
		...
		bSuccessFlag = True
		if iPlayersActual > 18:
			bUseDefaultStartPlacement = True
			iPlayers = 18
		else:
			bUseDefaultStartPlacement = False
			iPlayers = iPlayersActual
...
class WheelMultilayeredFractal(CvMapGeneratorUtil.MultilayeredFractal):
	...
	def generateCenter(self, iWaterPercent, iRegionWidth, iRegionHeight, iRegionWestX, iRegionSouthY, iRegionGrain, iRegionHillsGrain, iRegionPlotFlags, iRegionTerrainFlags, iRegionFracXExp = -1, iRegionFracYExp = -1, bShift = True, iStrip = 15, rift_grain = -1, has_center_rift = False, invert_heights = False):
		# This is the code to generate each fractal.
		# Determine and pass in the appropriate arguments from the controlling function.
		#
		# Init local variables
...
def assignStartingPlots():
	...
	shuffledPlayers = []
	for playerLoopTwo in range(min(iPlayers, len(player_list))):
		iChoosePlayer = dice.get(len(player_list), "Shuffling Template IDs - Hub PYTHON")
		iShuffledPlayer = player_list[iChoosePlayer]
		shuffledPlayers.append(iShuffledPlayer)
		playerTemplateByID[iShuffledPlayer] = playerLoopTwo
...
def minStartingDistanceModifier():
	global bSuccessFlag
	global bUseDefaultStartPlacement
	if bSuccessFlag == True and bUseDefaultStartPlacement == False:
		return -95
	else:
		return -50
...
def findStartingPlot(argsList):
	...
	def isValid(playerID, x, y):
		...
		global iPlotShift
		global playerTemplateByID
		if not playerTemplateByID.has_key(playerID):
			return true
