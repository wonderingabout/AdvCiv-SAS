...
Note 4: if a new version of this/these file(s) is released for example in AdvCiv-SAS, remember to update all of them, not just the one you run (i.e. do not just update only generate_leaders_data.py to latest new version in AdvCiv-SAS or in another mod, but also update all files it depends on such as ai_utils_shared_with_civ4.py (see link above anyways etc) in your mod too, else you may run into errors, but if the dependency file(s) such as ai_utils_shared_with_civ4.py or/and other ones if any more exist, did not change, then just updating the files that changed/got a new update/version is fine/enough i would say (so just updating generate_leaders_data py file would be enough then)). In doubt and easiest, is to always update all files, both generate_leaders_data.py but also any file/dependency it depends on/uses such as ai_utils_shared_with_civ4.py in your mod folder(s). You can find such a list of dependency file(s) in generate_leaders_data.py imports, around/in lines like these anyways etc:

```
from Assets.Python.Contrib.Sevopedia.ai_utils_shared_with_civ4 import *
```

As of now ai_utils_shared_with_civ4.py is the only dependency we use in AdvCiv-SAS (excluding things like json, sys, os, datetime, etc that don't depend on us and that you don't have to update here if i'm not mistaken anyways etc) if i'm not mistaken, so just updating it in your mod (and place it at expected folder, in doubt same path tree (like /Assets/Python/etc... same as in AdvCiv-SAS to the file) as where it is in AdvCiv-SAS but in your own mod) while you also update generate_leaders_data.py should be plenty, but if other such dependencies were to be added, you'd need to update them as well if they get a new version, and again in doubt best to update all even though they shouldn't change often but they may from time to time or may not, so update all and done i would say in this case of mod reuse of our scripts/files at least, hopefully clear and not too (un?)disgestible and helpful but anyways etc...
